...
export type LabReportRow = { test: string; normal?: string; unit?: string; value?: string; prevValue?: string; flag?: 'normal'|'high'|'low'|'abnormal'|'critical'; comment?: string; isSection?: boolean }
...
function pickColumns(rows: LabReportRow[]) {
  ...
  const body = nonEmptyRows.map(r => {
    if (r.isSection) {
      return [{ content: (r.test || '').toUpperCase(), colSpan: colCount, styles: { fontStyle: 'bold', fillColor: [241, 245, 249], textColor: [0, 51, 102], halign: 'left' } }]
    }
    return [
