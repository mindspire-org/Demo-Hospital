...
export const labApi = {
  ...
  listPurchaseOrders: (params?: { q?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/lab/purchase-orders${s ? `?${s}` : ''}`)
  },
  ...
  updatePurchaseOrderStatus: (id: string, status: string) => api(`/lab/purchase-orders/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  ...
  assignSupplierCompanies: (supplierId: string, data: { companyIds?: string[]; unassignIds?: string[] }) => api(`/lab/suppliers/${supplierId}/companies`, { method: 'POST', body: JSON.stringify(data) }),
  ...
  updateOrderToken: (tokenNo: string, data: { tests: string[]; discount: number; receivedAmount: number }) =>
  ...
  listResults: (params?: { orderId?: string; from?: string; to?: string; reportStatus?: string; page?: number; limit?: number }) => {
    ...
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    if (params?.reportStatus) qs.set('reportStatus', params.reportStatus)
    if (params?.page != null) qs.set('page', String(params.page))
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
  ...
  listTokens: (params?: { q?: string; status?: 'token_generated' | 'converted_to_sample' | 'sample_received' | 'result_entered' | 'approved' | 'cancelled'; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.status) qs.set('status', params.status)
    if (params?.from) qs.set('from', params.from)
...
export const hospitalApi = {
  ...
  listStorePurchaseDrafts: (params?: { search?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/store/purchase-drafts${s ? `?${s}` : ''}`)
  },
  listStorePurchaseDraftLines: (params?: { search?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/store/purchase-drafts/lines${s ? `?${s}` : ''}`)
  },
  getStoreNextInvoiceNumber: () =>
    api('/hospital/store/purchase-drafts/next-invoice'),
  getStorePurchaseDraft: (id: string) =>
    api(`/hospital/store/purchase-drafts/${encodeURIComponent(id)}`),
  createStorePurchaseDraft: (data: any) =>
    api('/hospital/store/purchase-drafts', { method: 'POST', body: JSON.stringify(data) }),
  updateStorePurchaseDraft: (id: string, data: any) =>
    api(`/hospital/store/purchase-drafts/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteStorePurchaseDraft: (id: string) =>
    api(`/hospital/store/purchase-drafts/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  approveStorePurchaseDraft: (id: string) =>
    api(`/hospital/store/purchase-drafts/${encodeURIComponent(id)}/approve`, { method: 'POST' }),
