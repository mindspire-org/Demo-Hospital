...
export async function buildErReferralPdf(data: ErReferralPdfData){
  ...
  pdf.text('Emergency Referral', 14, y)
  pdf.setFont('helvetica', 'normal'); pdf.setFontSize(10)
  if (data.referral?.referredBy) pdf.text(`Referred by: ${data.referral.referredBy}`, pageWidth-14, y, { align: 'right' })
  y += 6

  // Serial and Priority (top right)
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(10)
  if (data.referral?.serial) {
    pdf.text(`Serial: ${data.referral.serial}`, pageWidth-14, y, { align: 'right' })
    y += 5
  }
  if (data.referral?.priority) {
    const priorityColor: any = { Critical: [220,38,38], Urgent: [217,119,6], Regular: [100,116,139] }
    const color = priorityColor[data.referral.priority] || priorityColor.Regular
    pdf.setTextColor(color[0], color[1], color[2])
    pdf.text(`Priority: ${data.referral.priority}`, pageWidth-14, y, { align: 'right' })
    pdf.setTextColor(0,0,0)
    y += 5
  }
  y += 2

  // Patient block - 3 column layout
  ...
  // Vitals, Referred To, Condition - 3 columns
...
export async function previewErReferralPdf(data: ErReferralPdfData){
  const pdf = await buildErReferralPdf(data)
  ...
  title.textContent = 'ER Referral Preview'
