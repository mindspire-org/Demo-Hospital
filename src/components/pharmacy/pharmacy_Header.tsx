...
export default function Pharmacy_Header({ onToggleSidebar, onToggleTheme, theme, variant = 'default' }: Props) {
  ...
  useEffect(() => {
    ...
    // Load display name from localStorage (pharmacy specific)
    try {
  ...
  async function handleLogout(){
    ...
    try { localStorage.removeItem('pharmacy.user'); localStorage.removeItem('pharmacy.token') } catch {}
      ...
      <div className={innerCls}>
        ...
        <Link to="/pharmacy" className="flex items-center gap-2">
          ...
          <div className="flex flex-col leading-tight">
            <div className={brandTitleCls.replace('HealthSpire', '') + ' ' + (isNavy ? 'text-white' : 'text-slate-900')}>Pharmacy Portal</div>
            <div className="text-[10px] uppercase tracking-wider text-white/60">{pharmacyName} Services</div>
          </div>
