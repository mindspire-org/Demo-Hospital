...
const accountingSection: Section = {
  ...
  items: [
    ...
    { to: '/finance/cash-handover', label: 'Cash Handover', icon: Receipt },
    { to: '/finance/pending-handovers', label: 'Pending Handovers', icon: Repeat },
    { to: '/finance/user-accounts', label: 'User Accounts', icon: UserCircle },
  ],
}
...
export default function Finance_Sidebar({ collapsed = false }: { collapsed?: boolean }) {
  ...
  const canShow = (path: string) => {
    if (path === '/finance/sidebar-permissions' && String(role || '').toLowerCase() !== 'admin') return false
