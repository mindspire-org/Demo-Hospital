...
import { Menu, Sun, Moon, LogOut, FlaskConical } from 'lucide-react'
...
export default function Lab_Header({ onToggleSidebar, collapsed, onToggleTheme, theme, variant = 'default' }: Props){
  const navigate = useNavigate()
  const [username, setUsername] = useState<string>('Lab User')
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const name = parsed?.username || parsed?.name || parsed?.user?.username || 'Lab User'
  ...
  function handleToggleTheme(){
    ...
    try {
      const scope = document.querySelector('.lab-scope')
    ...
    <header className={headerCls}>
      <div className={innerCls}>
        ...
        <Link to="/lab" className="flex items-center gap-2">
          <div className={`flex h-9 w-9 items-center justify-center rounded-full shadow-inner ${isNavy ? 'bg-white/20 text-white' : 'bg-violet-100 text-violet-600 dark:bg-violet-950/30 dark:text-violet-400'}`}>
            <FlaskConical className="h-5 w-5" />
          </div>
          <div className="flex flex-col leading-tight">
            <div className={titleCls}>HealthSpire</div>
            <div className={`text-[10px] uppercase tracking-wider ${isNavy ? 'text-white/60' : 'text-slate-500 dark:text-slate-400'}`}>Lab Management System</div>
          </div>
          <span className={pillCls}>Online</span>
        </Link>

        <div className="ml-auto flex items-center gap-3 text-sm">
          <LabNotificationsBell />

          <div className="hidden items-center gap-2 sm:flex">
             <div className={`flex items-center gap-2 rounded-lg px-3 py-1.5 border ${isNavy ? 'bg-white/5 border-white/10 text-white' : 'bg-slate-50 border-slate-200 dark:bg-slate-800/40 dark:border-slate-700 text-slate-700 dark:text-slate-300'}`}>
                ...
                <div className="flex flex-col">
                  <span className={`text-[10px] uppercase font-bold ${isNavy ? 'text-white/40' : 'text-slate-400 dark:text-slate-500'}`}>Today</span>
             ...
             <div className={`flex items-center gap-2 rounded-lg px-3 py-1.5 border ${isNavy ? 'bg-white/5 border-white/10 text-white' : 'bg-slate-50 border-slate-200 dark:bg-slate-800/40 dark:border-slate-700 text-slate-700 dark:text-slate-300'}`}>
                ...
                <div className="flex flex-col">
                  <span className={`text-[10px] uppercase font-bold ${isNavy ? 'text-white/40' : 'text-slate-400 dark:text-slate-500'}`}>Local Time</span>
