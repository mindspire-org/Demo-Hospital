...
import {
  ...
  ChevronDown, ChevronRight, FilePlus, Building2, Clock
} from 'lucide-react'
...
const nav: NavItem[] = [
  ...
  { to: '/lab/today-tokens', label: "Token History", icon: Clock },
      ...
      try{
        ...
        const computed = nav
          .filter(item => {
            ...
            // Admin: default to visible unless explicitly set to false
            // Others: default to hidden unless explicitly set to true
            const isAdmin = String(role||'').toLowerCase() === 'admin'
            if (perm) {
              return isAdmin ? perm.visible !== false : perm.visible === true
            }
            return isAdmin // Admin sees all by default, others see none by default
          })
          .sort((a,b)=>{
