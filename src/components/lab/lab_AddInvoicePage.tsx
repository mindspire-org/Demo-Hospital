...
import Lab_AddCompanyDialog from './lab_AddCompanyDialog'
...
export default function Lab_AddInvoicePage() {
  ...
  const autofillFromInventoryByName = async (name: string | undefined, rowId: string) => {
    ...
    if (!q) return
    try {
      const res: any = await labApi.listInventory({ search: q, limit: 1 })
  ...
  const addCompany = async (c: any) => {
    try {
      const created: any = await labApi.createCompany({
        name: c.name,
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const res: any = await labApi.listCompanies({ distributorId: supplierId })
        if (!mounted) return
  ...
  useEffect(() => {
    ...
    ;(async () => {
      if (!isEdit || !id) return
      try {
        const doc: any = await labApi.getPurchaseDraft(id)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await labApi.getNextPurchaseInvoiceNumber()
        if (!mounted) return
  ...
  const refreshCompanies = async (distId: string) => {
    const res: any = await labApi.listCompanies({ distributorId: distId })
    const list = res?.items ?? res ?? []
    setCompanies(list)
    return list as any[]
  }
  ...
  const addSupplier = async (s: any) => {
    try {
      const created: any = await labApi.createSupplier({
        name: s.name,
        company: s.company,
        phone: s.phone,
        address: s.address,
  ...
  useEffect(() => {
    ...
    labApi.listInventory({ limit: 1000 }).then((res: any) => {
      if (!mounted) return
      const itemList = (res?.items || []).map((it: any) => ({ id: it._id || it.id, name: it.name }))
      if (Array.isArray(itemList)) setAllItems(itemList)
    }).catch(() => {})
    return () => { mounted = false }
  }, [])

  const searchItems = async (query: string, rowId: string) => {
    if (!query.trim()) {
      setSuggestions([])
    ...
    try {
      const res: any = await labApi.listInventory({ search: query, limit: 20 })
      if (res?.items && Array.isArray(res.items)) {
        setSuggestions(res.items.map((it: any) => ({ id: it._id || it.id, name: it.name })))
        setShowSuggestions(rowId)
      }
    } catch {}
  }

  const handleItemInput = (value: string, rowId: string) => {
  ...
  const selectSuggestion = (suggestion: { id: number; name: string }, rowId: string) => {
    ...
    ;(async () => {
      try {
        const res: any = await labApi.listInventory({ search: suggestion.name, limit: 1 })
        const it = (res?.items || [])[0]
        if (!it) return
        const units = Number(it.unitsPerPack || 1)
        const directBuyUnit = (it.lastBuyPerUnit != null) ? Number(it.lastBuyPerUnit) : undefined
  ...
  const handleHoldInvoice = async () => {
    ...
    try {
      await labApi.createHoldPurchaseInvoice({
        invoiceNo,
        invoiceDate,
        supplierId,
        supplierName,
  ...
  const loadHeldInvoices = async () => {
    setHeldLoading(true)
    try {
      const res: any = await labApi.listHoldPurchaseInvoices()
  ...
  const loadHeldInvoice = async (id: string) => {
    try {
      const doc: any = await labApi.getHoldPurchaseInvoice(id)
      if (!doc) return
      ...
      setInvoiceTaxes(taxes)
      await labApi.deleteHoldPurchaseInvoice(id)
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate(fromPending ? '/lab/inventory?tab=pending' : '/lab/inventory')}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={async () => {
                  ...
                  if (!lines.length) {
                    showToast('error', 'Add at least one item with Qty (Packs)')
                    return
                  }
                  ...
                  if (invalidLine) {
                    showToast('error', 'Each item must have Qty (Packs) and Units/Pack greater than 0')
                    return
                  }
                  ...
                  try {
                    ...
                    if (isEdit && id) {
                      await labApi.updatePurchaseDraft(id, payload)
                      showToast('success', 'Invoice updated')
                    } else {
                      await labApi.createPurchaseDraft(payload)
                      showToast('success', 'Invoice saved successfully')
                    }
                    setTimeout(() => navigate(fromPending ? '/lab/inventory?tab=pending' : '/lab/inventory'), 350)
                  } catch (error) {
      ...
      <div className="mx-auto max-w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:bg-slate-800">
              <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Items ({items.length})</h2>
                  <button
                    type="button"
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            ...
                            <div className="relative lg:col-span-2">
                              <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Item Name *</label>
                              <input
                                list="lab-item-list"
                                value={row.name || ''}
                                onChange={e => handleItemInput(e.target.value, row.id)}
                                onFocus={() => {
                                  if (row.name && row.name.trim()) {
                                    searchItems(row.name, row.id)
                                  }
                                }}
                            ...
                            <div>
                              ...
                              <input 
                                ...
                                onChange={e => setItems(prev => prev.map(it => it.id === row.id ? { ...it, unitsPerPack: Number(e.target.value || 0), buyPerUnit: (row.buyPerPack || 0) / Math.max(1, Number(e.target.value || 0)), totalItems: (row.packs || 0) * Math.max(1, Number(e.target.value || 0)) } : it))} 
                                className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" 
                              />
                            </div>
          ...
          <div className="space-y-6">
            ...
            {/* Totals Summary */}
            <div className="bg-linear-to-br from-emerald-50 to-emerald-100 rounded-xl shadow-sm border border-emerald-200">
