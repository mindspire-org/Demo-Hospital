...
export default function Lab_AssignSupplierCompaniesDialog({ open, onClose, supplier }: { open: boolean; onClose: ()=>void; supplier: Supplier | null }){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const all: any = await labApi.listCompanies({})
        ...
        if (ids.length){
          try {
            const res: any = await labApi.listSuppliers()
  ...
  const save = async () => {
    ...
    try {
      ...
      await labApi.assignSupplierCompanies(supplier.id, { companyIds: toAssign, unassignIds: toUnassign })
      try { window.dispatchEvent(new Event('lab:companies:refresh')) } catch {}
      onClose()
