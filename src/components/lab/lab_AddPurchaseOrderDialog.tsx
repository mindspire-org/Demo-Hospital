...
const Lab_AddPurchaseOrderDialog: React.FC<Props> = ({ open, onClose, onSave, initial }) => {
  ...
  const [supplierCompanies, setSupplierCompanies] = useState<any[]>([]);
    ...
    if (open) {
      ...
      if (initial) {
        ...
        if (initial.supplierId) {
          loadCompaniesForSupplier(initial.supplierId);
        } else {
  ...
  const loadSuppliers = async () => {
    try {
      const res = await labApi.listSuppliers({ limit: 500 });
  ...
  const loadCompaniesForSupplier = async (id: string) => {
    ...
    try {
      const res: any = await labApi.listCompanies({ distributorId: sid });
      const list: any[] = res?.items ?? res ?? [];
      setSupplierCompanies(list);
      // If current companyName is empty, default to first assigned company.
      if (!companyName && list.length) {
        const first = String(list[0]?.name || '').trim();
        if (first) setCompanyName(first);
      }
    } catch (e) {
      console.error('Failed to load companies for supplier', e);
      setSupplierCompanies([]);
    }
  };
  ...
  const loadInventory = async () => {
    try {
      const res = await labApi.listInventory({ limit: 1000 });
  ...
  const handleItemNameChange = (index: number, value: string) => {
    const newItems = [...items];
    newItems[index] = { ...newItems[index], name: value, itemId: undefined };
    setItems(newItems);
  ...
  const handleSave = () => {
    ...
    if (validItems.length === 0) {
      setToast({ message: 'Please add at least one item with a name.', type: 'error' });
      setTimeout(() => setToast(null), 3000);
      return;
    }
    onSave({
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {supplierId && supplierCompanies.length > 0 ? (
                  <select
                    value={companyName}
                  ...
                  >
                    ...
                    {supplierCompanies.map((c: any) => (
                      <option key={String(c._id || c.id || c.name)} value={String(c.name || '')}>
                        {String(c.name || '')}
                      </option>
                    ))}
                  </select>
                      ...
                      .map(s => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setCompanyName('');
                            setSupplierCompanies([]);
                            loadCompaniesForSupplier(s._id);
                            ...
                            <div className="inventory-dropdown absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-md border border-slate-200 bg-white shadow-xl">
                              {(itemSearchQuery
                                ...
                                >
                                  ...
                                  <div className="text-xs text-slate-500">{inv.category || 'General'} • On Hand: {inv.onHand || 0}</div>
                                </div>
                              ))}
...
export default Lab_AddPurchaseOrderDialog;
