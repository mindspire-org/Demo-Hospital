...
export default function IndoorPharmacy_Header({ onToggleSidebar, onToggleTheme, theme, variant = 'default' }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem('user') || localStorage.getItem('indoorpharmacy.user')
      if (raw) {
  ...
  async function handleLogout(){
    ...
    try { localStorage.removeItem('user'); localStorage.removeItem('indoorpharmacy.user'); localStorage.removeItem('indoorpharmacy.token') } catch {}
      ...
      <div className={innerCls}>
        ...
        <Link to="/indoor-pharmacy" className="flex items-center gap-2">
          ...
          <div className="hidden sm:block">
            <div className={brandNameCls}>{indoorPharmacyName}</div>
            <div className={brandTitleCls}>HealthSpire</div>
          </div>
          <div className={mobileTitleCls}>{indoorPharmacyName}</div>
