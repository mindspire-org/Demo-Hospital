...
export default function Hospital_ErMedication({ encounterId }: { encounterId: string }){
  ...
  async function save(items: Array<{ name: string; dose: string; route: string; freq: string; duration: string; prescribedBy: string; prescribingDoctorId: string }>) {
    try {
      for (const d of items) {
        ...
        await hospitalApi.createErMedOrder(encounterId, { drugName: name, dose, route, frequency: freq, duration, prescribedBy, prescribingDoctorId })
      }
      setOpen(false)
      await reload()
    } catch (e: any) {
  ...
  async function update(d: { name: string; dose: string; route: string; freq: string; duration: string; prescribedBy: string; prescribingDoctorId: string }) {
    if (!editingId) return
    try {
      await hospitalApi.createErMedOrder(encounterId, { drugName: d.name, dose: d.dose, route: d.route, frequency: d.freq, duration: d.duration, prescribedBy: d.prescribedBy, prescribingDoctorId: d.prescribingDoctorId })
      await hospitalApi.deleteErMedOrder(editingId)
  ...
  const stopMed = async (id: string) => {
    try {
      await hospitalApi.stopErMedOrder(id)
      await reload()
    } catch (e: any) {
  ...
  const handleExecuteSave = async (data: { quantity: number; remarks: string; executedAt: string }) => {
    ...
    try {
      await hospitalApi.executeErMedOrder(executingOrder.id, data)
      setExecOpen(false)
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteId) return
    try { await hospitalApi.deleteErMedOrder(deleteId); await reload() } catch (e: any) { alert(e?.message || 'Failed to delete') }
    setDeleteId(null)
  }
