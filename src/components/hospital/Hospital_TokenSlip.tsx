  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 print:bg-white print:static">
      <div id="hospital-receipt" className="max-h-[80vh] w-[360px] overflow-y-auto rounded-md border border-slate-300 bg-white p-4 shadow print:max-h-none print:overflow-visible print:shadow-none print:border-0 print:w-[300px]">
        ...
        <div className="my-2 rounded border border-slate-800 p-3 text-center text-xl font-extrabold tracking-widest">{data.tokenNo}</div>

        <div className="space-y-1 text-sm text-slate-800">
          {data.mrn && <Row label="MR #:" value={data.mrn} />}
          <Row label="Patient Name:" value={data.patientName || '-'} />
          {(data.guardianName || data.guardianRel) && <Row label="Guardian Name:" value={`${data.guardianRel ? data.guardianRel + ' ' : ''}${data.guardianName || ''}`.trim()} />}
          ...
          {typeof data.corporateCoPayPercent === 'number' && <Row label="Co-Pay %:" value={`${data.corporateCoPayPercent}%`} />}
        </div>
      ...
      <style>{`
        ...
        @media print {
          ...
          #hospital-receipt { position: absolute !important; left: 0; right: 0; top: 0; margin: 0 auto !important; padding: 10px 10px 0 10px !important; width: 300px !important; box-sizing: border-box !important; line-height: 1.45; overflow: visible !important; z-index: 2147483647; font-size: 14px !important; background: #fff !important; }
          #hospital-receipt, #hospital-receipt * { color: #000 !important; background: transparent !important; background-color: transparent !important; border-color: #000 !important }
