...
import GynecologicalObstetricExam from './Hospital_GynecologicalObstetricExam'

export default function Hospital_IpdGynecologicalObstetricSystem({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdClinicalNotes(encounterId, { type: 'gynecological-obstetric-exam', limit: 200 }) as any
  ...
  const add = async (d: any) => {
    try{
      ...
      await hospitalApi.createIpdClinicalNote(encounterId, {
        type: 'gynecological-obstetric-exam',
        sign: '',
        data: d,
      })
      setOpen(false)
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <GynecologicalObstetricExamDisplay data={r.data} />
...
function GynecologicalObstetricExamDisplay({ data }: { data: any }) {
  const exam = data || {}
