...
export default function Hospital_IpdBloodTransfusionNotes({ encounterId }: { encounterId: string }) {
  ...
  const [editingData, setEditingData] = useState<RecordItem | null>(null)
  ...
  async function update(form: any) {
    ...
    setError(null)
    try {
      await ipdApi.updateIpdBloodTransfusion(editingId, {
        indication: form.adverseReaction || 'Blood transfusion',
        issueDateTime: form.issueDateTime || undefined,
        screeningResults: form.screeningResults || undefined,
        preTransfusionVitals: {
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteId) return
    try { await ipdApi.deleteIpdBloodTransfusion(deleteId); await reload() } catch (e: any) { alert(e?.message || 'Failed to delete') }
    setDeleteId(null)
  }

  function startEdit(it: RecordItem) {
    setEditingId(it.id)
    setEditingData(it)
    setOpen(true)
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <BloodTransfusionNotesDialog key={editingId || 'add'} open={open} onClose={() => { setOpen(false); setEditingId(null); setEditingData(null) }} onSave={editingId ? update : save} initial={editingData} />
      <ConfirmDialog
        open={deleteConfirmOpen}
        title="Delete Blood Transfusion Record"
        message="Are you sure you want to delete this blood transfusion record?"
...
function BloodTransfusionNotesDialog({
  ...
  const [form, setForm] = useState({
    ...
    labNo: p?.labNo || '',
    issueDateTime: p?.issueDateTime || new Date().toISOString().slice(0, 16),
    screeningResults: p?.screeningResults || '',
    prePulse: p?.prePulse || '',
    preBP: p?.preBP || '',
    preTemp: p?.preTemp || '',
    preRespRate: p?.preRespRate || '',
    preChest: p?.preChest || '',
    crossMatchSalinePhase: p?.crossMatchSalinePhase || '',
    crossMatchAlbuminPhase: p?.crossMatchAlbuminPhase || '',
    receivedInWard: p?.receivedInWard || '',
    transfusionStartedAt: p?.transfusionStartedAt || '',
    postPulse: p?.postPulse || '',
    postBP: p?.postBP || '',
    postTemp: p?.postTemp || '',
    postRespRate: p?.postRespRate || '',
    postChest: p?.postChest || '',
    transfusionCompletedAt: p?.transfusionCompletedAt || '',
    adverseReaction: p?.adverseReaction || '',
    medications: p?.medications || '',
    doctorStamp: p?.doctorStamp || '',
    cnicNumber: p?.cnicNumber || '',
    signatureThumb: p?.signatureThumb || '',
  })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl bg-white p-6">
        <h3 className="mb-4 text-lg font-semibold">{initial ? 'Edit Blood Transfusion Notes' : 'Blood Transfusion Notes'}</h3>
