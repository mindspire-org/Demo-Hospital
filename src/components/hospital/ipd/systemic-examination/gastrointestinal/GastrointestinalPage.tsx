...
export default function GastrointestinalPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdSystemicExams(encounterId, { systemType: 'gastrointestinal', limit: 200 }) as any
      const items = (res.systemicExams || []).map((n: any) => ({
        id: String(n._id),
        recordedAt: String(n.recordedAt || n.createdAt || ''),
        data: n.gastrointestinal || n.data || {},
      }))
      setRecords(items)
    }catch{}
  }
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <GastrointestinalDisplay data={r.data} />
...
function GastrointestinalDisplay({ data }: { data: any }) {
  const gi = data || {}
  ...
  return (
    <div className="space-y-3 border border-slate-300 p-3 rounded-lg bg-slate-50 text-xs">
      ...
      {gi.bowelFrequency && (<div className="border border-slate-300 p-2 bg-white"><div className="font-semibold mb-1">Bowel Frequency:</div><div>{gi.bowelFrequency}</div></div>)}
      ...
      {gi.others && (<div className="border border-slate-300 p-2 bg-white"><div className="font-semibold mb-1">Others:</div><div>{gi.others}</div></div>)}
    </div>
  )
}
