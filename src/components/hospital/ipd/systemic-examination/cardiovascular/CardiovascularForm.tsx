export default function CardiovascularForm({ data, onChange }: { data: any; onChange: (data: any) => void }) {
  const cvs = data || {}
      ...
      <div className="border border-slate-300 p-2 space-y-2">
        ...
        <div className="border-t border-slate-300 pt-2 mt-2">
          ...
          <table className="w-full border border-slate-300 text-xs">
            ...
            <tbody>
              {(['carotid', 'femoral', 'popliteal', 'dorsalisPedis'] as const).map((area) => (
                <tr key={area}>
                  <td className="border border-slate-300 px-2 py-1 capitalize">{area.replace(/([A-Z])/g, ' $1').trim()}</td>
                  {(['right', 'left'] as const).map((side) => (
                    <td key={side} className="border border-slate-300 px-2 py-1 text-center">
                      <input
                        type="checkbox"
                        checked={cvs.auscultation?.peripheralPulses?.[area]?.[side] || false}
                        onChange={(e) => {
                          const newAusc = {
                            ...(cvs.auscultation || {}),
                            peripheralPulses: {
                              ...(cvs.auscultation?.peripheralPulses || {}),
                              [area]: { ...(cvs.auscultation?.peripheralPulses?.[area] || {}), [side]: e.target.checked }
                            }
                          }
                          onChange({ ...cvs, auscultation: newAusc })
                        }}
