...
export default function RespiratoryPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdSystemicExams(encounterId, { systemType: 'respiratory', limit: 200 }) as any
      const items = (res.systemicExams || []).map((n: any) => ({
        id: String(n._id),
        recordedAt: String(n.recordedAt || n.createdAt || ''),
        data: n.respiratory || n.data || {},
      }))
      setRecords(items)
    }catch{}
  }
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <RespiratoryDisplay data={r.data} />
...
function RespiratoryDisplay({ data }: { data: any }) {
  const resp = data || {}
