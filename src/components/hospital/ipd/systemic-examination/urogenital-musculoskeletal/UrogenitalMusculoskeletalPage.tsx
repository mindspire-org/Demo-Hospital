...
import UrogenitalMusculoskeletalForm from './UrogenitalMusculoskeletalForm'

export default function UrogenitalMusculoskeletalPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdSystemicExams(encounterId, { systemType: 'urogenital-musculoskeletal', limit: 200 }) as any
      const items = (res.systemicExams || []).map((n: any) => ({
        id: String(n._id),
        recordedAt: String(n.recordedAt || n.createdAt || ''),
        data: n.urogenitalMusculoskeletal || n.data || {},
      }))
      setRecords(items)
    }catch{}
  }
  ...
  const add = async (d: any) => {
    try{
      ...
      await hospitalApi.createIpdSystemicExam(encounterId, {
        systemType: 'urogenital-musculoskeletal',
        urogenitalMusculoskeletal: d,
      })
      setOpen(false)
      await reload()
    }catch(e: any){ 
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <UrogenitalMusculoskeletalDisplay data={r.data} />
...
function UrogenitalMusculoskeletalDisplay({ data }: { data: any }) {
  const exam = data || {}
    ...
    <div className="space-y-3 border border-slate-300 p-3 rounded-lg bg-slate-50 text-xs">
      ...
      {exam.urogenital && (
        <div className="border border-slate-300 p-2 bg-white">
          ...
          {exam.urogenital.genitalAnomaly && (<div className="mt-2"><div className="font-semibold">Genital Anomaly:</div><div>{exam.urogenital.genitalAnomaly}</div></div>)}
          ...
          {exam.urogenital.others && (<div className="mt-2"><div className="font-semibold">Others:</div><div>{exam.urogenital.others}</div></div>)}
        </div>
      )}
      ...
      {exam.musculoskeletal && (
        <div className="border border-slate-300 p-2 bg-white">
          ...
          {exam.musculoskeletal.rangeType && (<div className="flex flex-wrap gap-2">{exam.musculoskeletal.rangeType.passive && <div>✓ Passive</div>}{exam.musculoskeletal.rangeType.restricted && <div>✓ Restricted</div>}</div>)}
          {exam.musculoskeletal.rangeOfMovement?.location && (<div className="mt-1">Range of Movement Location: {exam.musculoskeletal.rangeOfMovement.location}</div>)}
          {exam.musculoskeletal.weaknessParalysis?.location && (<div className="mt-1">Weakness/Paralysis Location: {exam.musculoskeletal.weaknessParalysis.location}</div>)}
          {exam.musculoskeletal.contractures?.location && (<div className="mt-1">Contractures Location: {exam.musculoskeletal.contractures.location}</div>)}
          {exam.musculoskeletal.deformities?.location && (<div className="mt-1">Deformities Location: {exam.musculoskeletal.deformities.location}</div>)}
          {exam.musculoskeletal.fracture && (
            <div className="mt-2">
              <div className="font-semibold">Fracture:</div>
              <div className="flex flex-wrap gap-2">{exam.musculoskeletal.fracture.new && <div>✓ New</div>}{exam.musculoskeletal.fracture.splint && <div>✓ Splint</div>}</div>
            </div>
          )}
          {exam.musculoskeletal.ambulatoryDevices && (
            <div className="mt-2">
              <div className="font-semibold">Ambulatory Devices:</div>
              <div className="flex flex-wrap gap-2">{exam.musculoskeletal.ambulatoryDevices.withPatient && <div>✓ With Patient</div>}{exam.musculoskeletal.ambulatoryDevices.atHome && <div>✓ At Home</div>}</div>
            </div>
          )}
          {exam.musculoskeletal.limb && (
            <div className="mt-2">
              <div className="font-semibold">Limb:</div>
              <div className="flex flex-wrap gap-2">{exam.musculoskeletal.limb.cast && <div>Cast: {exam.musculoskeletal.limb.cast}</div>}{exam.musculoskeletal.limb.other && <div>Other: {exam.musculoskeletal.limb.other}</div>}</div>
            </div>
          )}
          {exam.musculoskeletal.back && (
            <div className="mt-2">
              <div className="font-semibold">Back:</div>
              <div className="flex flex-wrap gap-2">{exam.musculoskeletal.back.sacralDimple && <div>✓ Sacral Dimple</div>}{exam.musculoskeletal.back.kyphosis && <div>✓ Kyphosis</div>}{exam.musculoskeletal.back.lordosis && <div>✓ Lordosis</div>}{exam.musculoskeletal.back.scoliosis && <div>✓ Scoliosis</div>}{exam.musculoskeletal.back.other && <div>Other: {exam.musculoskeletal.back.other}</div>}</div>
            </div>
          )}
          {exam.musculoskeletal.joints && (
            <div className="mt-2">
              <div className="font-semibold">Joints:</div>
              <div className="flex flex-wrap gap-2">{exam.musculoskeletal.joints.mobile && <div>✓ Mobile</div>}{exam.musculoskeletal.joints.swelling && <div>✓ Swelling</div>}{exam.musculoskeletal.joints.tenderOrInflamedJoints && <div>✓ Tender or Inflamed Joints</div>}</div>
            </div>
          )}
          {exam.musculoskeletal.spine && (
            <div className="mt-2">
              <div className="font-semibold">Spine:</div>
              <div className="flex flex-wrap gap-2">{exam.musculoskeletal.spine.inTowing && <div>✓ In Towing</div>}{exam.musculoskeletal.spine.notTowing && <div>✓ Not Towing</div>}{exam.musculoskeletal.spine.bowType && <div>✓ Bow type</div>}{exam.musculoskeletal.spine.knockKnee && <div>✓ Knock knee</div>}{exam.musculoskeletal.spine.limp && <div>✓ Limp</div>}{exam.musculoskeletal.spine.knee && <div>✓ Knee</div>}</div>
            </div>
          )}
          {exam.musculoskeletal.others && (<div className="mt-2"><div className="font-semibold">Others:</div><div>{exam.musculoskeletal.others}</div></div>)}
        </div>
      )}
