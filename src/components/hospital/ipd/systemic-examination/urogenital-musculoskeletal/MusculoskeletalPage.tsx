...
import MusculoskeletalForm from './MusculoskeletalForm'

export default function MusculoskeletalPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdSystemicExams(encounterId, { systemType: 'musculoskeletal', limit: 200 }) as any
      const items = (res.systemicExams || []).map((n: any) => ({
        id: String(n._id),
        recordedAt: String(n.recordedAt || n.createdAt || ''),
        data: n.musculoskeletal || n.data || {},
      }))
      setRecords(items)
    }catch{}
  }
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <MusculoskeletalDisplay data={r.data} />
...
function MusculoskeletalDisplay({ data }: { data: any }) {
  const msk = data || {}
