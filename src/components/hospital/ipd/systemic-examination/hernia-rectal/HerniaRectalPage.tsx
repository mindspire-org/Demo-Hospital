...
import HerniaRectalForm from './HerniaRectalForm'

export default function HerniaRectalPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdSystemicExams(encounterId, { systemType: 'hernia-rectal', limit: 200 }) as any
      const items = (res.systemicExams || []).map((n: any) => ({
        id: String(n._id),
        recordedAt: String(n.recordedAt || n.createdAt || ''),
        data: n.herniaRectal || n.data || {},
      }))
      setRecords(items)
    }catch{}
  }
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <HerniaRectalDisplay data={r.data} />
...
function HerniaRectalDisplay({ data }: { data: any }) {
  const exam = data || {}
        ...
        <div className="border border-slate-300 p-2 bg-white">
          ...
          {exam.rectal.digitalExam && (
            <div className="mt-2">
              ...
              <div>{exam.rectal.digitalExam.performed && <div>✓ Performed</div>}{exam.rectal.digitalExam.notPerformed && <div>✓ Not Performed</div>}{exam.rectal.digitalExam.findings && <div className="mt-1">Findings: {exam.rectal.digitalExam.findings}</div>}</div>
            </div>
          )}
        </div>
      )}
