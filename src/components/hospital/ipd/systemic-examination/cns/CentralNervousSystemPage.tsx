...
import CentralNervousSystemForm from './CentralNervousSystemForm'

export default function CentralNervousSystemPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdSystemicExams(encounterId, { systemType: 'cns', limit: 200 }) as any
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4" data-encounterid={encounterId}>
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <CentralNervousSystemDisplay data={r.data} />
      ...
      <CentralNervousSystemDialog open={open} onClose={()=>setOpen(false)} onSave={add} />
    </div>
  )
}

function CentralNervousSystemDialog({
  open,
  onClose,
  onSave,
}: {
...
function CentralNervousSystemDisplay({ data }: { data: any }) {
  const cns = data || {}
      ...
      {cns.cranialNerves && (
        <div className="border border-slate-300 p-2 bg-white">
          ...
          <div className="space-y-1 text-xs">
            {cns.cranialNerves.olfactory && (<div>I-Olfactory: R: {cns.cranialNerves.olfactory.right || '-'} | L: {cns.cranialNerves.olfactory.left || '-'}</div>)}
            {cns.cranialNerves.optic && (<div><div>II-Optic:</div>{cns.cranialNerves.optic.visualAcuity && <div className="ml-2">Visual Acuity - R: {cns.cranialNerves.optic.visualAcuity.right || '-'} | L: {cns.cranialNerves.optic.visualAcuity.left || '-'}</div>}{cns.cranialNerves.optic.visualFields && <div className="ml-2">Visual Fields - R: {cns.cranialNerves.optic.visualFields.right || '-'} | L: {cns.cranialNerves.optic.visualFields.left || '-'}</div>}{cns.cranialNerves.optic.fundus && <div className="ml-2">Fundus - R: {cns.cranialNerves.optic.fundus.right || '-'} | L: {cns.cranialNerves.optic.fundus.left || '-'}</div>}</div>)}
            {cns.cranialNerves.oculomotor && (<div>III-IV-VI Oculomotor: R: {cns.cranialNerves.oculomotor.right || '-'} | L: {cns.cranialNerves.oculomotor.left || '-'}</div>)}
            {cns.cranialNerves.trigeminal && (<div>V-Trigeminal: R: {cns.cranialNerves.trigeminal.right || '-'} | L: {cns.cranialNerves.trigeminal.left || '-'}</div>)}
            {cns.cranialNerves.facial && (<div>VII-Facial: R: {cns.cranialNerves.facial.right || '-'} | L: {cns.cranialNerves.facial.left || '-'}</div>)}
            {cns.cranialNerves.vestibulocochlear && (<div>VIII-Vestibulocochlear: R: {cns.cranialNerves.vestibulocochlear.right || '-'} | L: {cns.cranialNerves.vestibulocochlear.left || '-'}</div>)}
            {cns.cranialNerves.glossopharyngeal && (<div>IX-Glossopharyngeal: R: {cns.cranialNerves.glossopharyngeal.right || '-'} | L: {cns.cranialNerves.glossopharyngeal.left || '-'}</div>)}
            {cns.cranialNerves.vagus && (<div>X-Vagus: R: {cns.cranialNerves.vagus.right || '-'} | L: {cns.cranialNerves.vagus.left || '-'}</div>)}
            {cns.cranialNerves.accessory && (<div>XI-Accessory: R: {cns.cranialNerves.accessory.right || '-'} | L: {cns.cranialNerves.accessory.left || '-'}</div>)}
            {cns.cranialNerves.hypoglossal && (<div>XII-Hypoglossal: R: {cns.cranialNerves.hypoglossal.right || '-'} | L: {cns.cranialNerves.hypoglossal.left || '-'}</div>)}
          </div>
        </div>
      )}
