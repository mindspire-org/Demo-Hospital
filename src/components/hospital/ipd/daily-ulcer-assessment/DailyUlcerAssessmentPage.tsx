...
import DailyUlcerAssessmentForm from './DailyUlcerAssessmentForm'

export default function DailyUlcerAssessmentPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdDailyUlcerAssessments(encounterId, { limit: 200 }) as any
      const items = (res.dailyUlcerAssessments || []).map((n: any) => ({
        id: String(n._id),
  ...
  const add = async (d: any) => {
    try{
      ...
      await hospitalApi.createIpdDailyUlcerAssessment(encounterId, d)
      setOpen(false)
      await reload()
    }catch(e: any){ 
          ...
          <table className="w-full border border-slate-300 text-xs">
            ...
            <tbody>
              <tr>
                ...
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.size || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Depth</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.depth || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Edge</td>
                {assessments.map((a: any, i: number) => (
                  <td key={i} className="border border-slate-300 px-2 py-1">{a.edge?.clearVisible && '✓ Clear '}{a.edge?.attachedToWoundBase && '✓ Attached '}{a.edge?.fibroticScarred && '✓ Fibrotic'}</td>
                ))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Undermining</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.undermining || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Necrotic Tissue</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.necroticTissue || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Slough</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.slough || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Eschar</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.eschar || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Exudate</td>
                {assessments.map((a: any, i: number) => (
                  <td key={i} className="border border-slate-300 px-2 py-1">{a.exudate?.serous && '✓ Serous '}{a.exudate?.serosanguinous && '✓ Serosang. '}{a.exudate?.purulent && '✓ Purulent'}</td>
                ))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Granulation</td>
                {assessments.map((a: any, i: number) => (
                  <td key={i} className="border border-slate-300 px-2 py-1">{a.granulation?.healthy && '✓ Healthy '}{a.granulation?.septic && '✓ Septic'}</td>
                ))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Epithelialization</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.epithelialization || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Surrounding Skin</td>
                {assessments.map((a: any, i: number) => (
                  <td key={i} className="border border-slate-300 px-2 py-1">{a.surroundingSkin?.brightRed && '✓ Red '}{a.surroundingSkin?.blanchable && '✓ Blanch '}{a.surroundingSkin?.edematous && '✓ Edema '}{a.surroundingSkin?.indurated && '✓ Indur.'}</td>
                ))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Stage</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.stageOfPressureUlcer || '-'}</td>))}
              </tr>
              <tr>
                <td className="border border-slate-300 px-2 py-1 font-semibold">Nurse Sign & ID</td>
                {assessments.map((a: any, i: number) => (<td key={i} className="border border-slate-300 px-2 py-1">{a.nurseSignatureId || '-'}</td>))}
              </tr>
            </tbody>
          </table>
        </div>
