...
import NicuEvaluationForm from './NicuEvaluationForm'
import { hospitalApi } from '../../../../utils/api'

export default function NicuEvaluationPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdNicuEvaluations(encounterId, { limit: 200 }) as any
      const items = (res.nicuEvaluations || []).map((n: any) => ({
        id: String(n._id),
  ...
  const add = async (d: any) => {
    try{
      ...
      await hospitalApi.createIpdNicuEvaluation(encounterId, d)
      setOpen(false)
      await reload()
    }catch(e: any){ 
