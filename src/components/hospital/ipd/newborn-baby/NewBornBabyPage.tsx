...
export default function NewBornBabyPage({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdNewBornBabyNotes(encounterId, { limit: 200 }) as any
      const items = (res.newBornBabyNotes || []).map((n: any) => ({
        id: String(n._id),
  ...
  const add = async (d: any) => {
    try{
      ...
      await hospitalApi.createIpdNewBornBabyNotes(encounterId, d)
      setOpen(false)
      await reload()
    }catch(e: any){ 
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4" data-encounterid={encounterId}>
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <NewBornBabyDisplay data={r.data} />
      ...
      <NewBornBabyDialog open={open} onClose={()=>setOpen(false)} onSave={add} />
    </div>
  )
}

function NewBornBabyDialog({
  open,
  onClose,
  onSave,
}: {
...
function NewBornBabyDisplay({ data }: { data: any }) {
  const exam = data || {}
