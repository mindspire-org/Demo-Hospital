...
export default function DailyMonitoring({ encounterId }: { encounterId: string }){
  ...
  async function save(d: { date?: string; time?: string; bp?: string; temp?: string; pulse?: string; resp?: string; bsr?: string; intakeIV?: string; urine?: string; nurseSign?: string; shift?: 'morning'|'evening'|'night' }){
    ...
    }catch(e: any){ setToast({ type: 'error', message: e?.message || 'Failed to save monitoring entry' }) }
  }

  async function update(d: { date?: string; time?: string; bp?: string; temp?: string; pulse?: string; resp?: string; bsr?: string; intakeIV?: string; urine?: string; nurseSign?: string; shift?: 'morning'|'evening'|'night' }){
    if (!editingId) return
    try{
      const recordedAt = (d.date && d.time) ? new Date(`${d.date}T${d.time}`).toISOString() : new Date().toISOString()
      await hospitalApi.updateIpdVital(editingId, {
        recordedAt,
        bp: d.bp,
        temp: d.temp ? parseFloat(d.temp) : undefined,
        hr: d.pulse ? parseFloat(d.pulse) : undefined,
    ...
    }catch(e: any){ setToast({ type: 'error', message: e?.message || 'Failed to update monitoring entry' }) }
  }
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteId) return
    try { await hospitalApi.deleteIpdVital(deleteId); await reload() } catch (e: any) { setToast({ type: 'error', message: e?.message || 'Failed to delete' }) }
    setDeleteId(null)
  }
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <div className="mb-2 flex items-center justify-between print:hidden">
        ...
        <div className="flex items-center gap-2">
          <button onClick={()=>setOpen(true)} className="btn">Add Vital Record</button>
        </div>
      </div>
      {rows.length === 0 ? (
                            ...
                            <tr key={v.id}>
                              ...
                              <td className="px-3 py-2 print:hidden">
                                <div className="flex gap-1">
                                  ...
                                  <button onClick={() => remove(v.id)} className="rounded px-2 py-0.5 text-xs text-red-600 hover:bg-red-50">Delete</button>
                                </div>
                              </td>
                            </tr>
                          ))}
      ...
      <VitalsDialog key={editingId || 'add'} open={open} onClose={()=>{ setOpen(false); setEditingId(null); setEditingData(null) }} onSave={editingId ? update : save} initial={editingData} />
      <ConfirmDialog
        open={deleteConfirmOpen}
        title="Delete Monitoring Entry"
        message="Are you sure you want to delete this monitoring entry?"
...
function VitalsDialog({ open, onClose, onSave, initial }: { open: boolean; onClose: ()=>void; onSave: (d: { date?: string; time?: string; bp?: string; temp?: string; pulse?: string; resp?: string; bsr?: string; intakeIV?: string; urine?: string; nurseSign?: string; shift?: 'morning'|'evening'|'night' })=>void; initial?: any }){
  if(!open) return null
  const defaultDate = initial?.date || new Date().toISOString().slice(0, 10)
  const defaultTime = initial?.time || new Date().toTimeString().slice(0, 5)
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const fd = new FormData(e.currentTarget)
    const shift = String(fd.get('shift') || '')
    if (!shift) {
      window.dispatchEvent(new CustomEvent('app:alert', { detail: { message: 'Please select a shift' } }))
      return
    }
    onSave({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <form onSubmit={submit} className="w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5">
        <div className="border-b border-slate-200 px-5 py-3 font-semibold text-slate-800">{initial ? 'Edit Daily Monitoring Entry' : 'Add Daily Monitoring Entry'}</div>
        <div className="px-5 py-4 text-sm">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div>
              ...
              <select id="mon-shift" name="shift" defaultValue={initial?.shift || ''} required className="w-full rounded-md border border-slate-300 px-3 py-2">
            ...
            <div>
              ...
              <input id="vital-bp" name="bp" defaultValue={initial?.bp || ''} placeholder="e.g. 120/80" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-pulse" className="block text-xs font-medium text-slate-600">Pulse</label>
              <input id="vital-pulse" name="pulse" defaultValue={initial?.pulse || ''} placeholder="e.g. 72" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-temp" className="block text-xs font-medium text-slate-600">Temp.</label>
              <input id="vital-temp" name="temp" defaultValue={initial?.temp || ''} placeholder="e.g. 98.6" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-resp" className="block text-xs font-medium text-slate-600">Resp.</label>
              <input id="vital-resp" name="resp" defaultValue={initial?.resp || ''} placeholder="e.g. 16" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-bsr" className="block text-xs font-medium text-slate-600">BSR</label>
              <input id="vital-bsr" name="bsr" type="number" step="0.1" defaultValue={initial?.bsr || ''} placeholder="e.g. 120" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-intake" className="block text-xs font-medium text-slate-600">Intake I/V</label>
              <input id="vital-intake" name="intakeIV" defaultValue={initial?.intakeIV || ''} placeholder="e.g. 500ml" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-urine" className="block text-xs font-medium text-slate-600">Urine</label>
              <input id="vital-urine" name="urine" defaultValue={initial?.urine || ''} placeholder="e.g. 300ml" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="vital-nurse" className="block text-xs font-medium text-slate-600">Nurse Sign</label>
              <input id="vital-nurse" name="nurseSign" defaultValue={initial?.nurseSign || ''} placeholder="Signature/Name" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
          </div>
        </div>
