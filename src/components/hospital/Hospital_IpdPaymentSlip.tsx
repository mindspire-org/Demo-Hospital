...
export type IpdPaymentSlipData = {
  encounterId: string
  patientName: string
  mrn?: string
  phone?: string
...
export default function Hospital_IpdPaymentSlip({ open, onClose, data, autoPrint = false, user }: { open: boolean; onClose: ()=>void; data: IpdPaymentSlipData; autoPrint?: boolean; user?: string }){
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 print:bg-white print:static">
      <div id="hospital-ipd-payment-receipt" className="w-[360px] rounded-md border border-slate-300 bg-white p-4 shadow print:shadow-none print:border-0 print:w-[300px]">
        ...
        <div className="text-center text-sm font-semibold underline">{data.isAdvanceReturn ? 'IPD Advance Return Receipt' : data.isAdvance ? 'IPD Advance Receipt' : 'IPD Payment Slip'}</div>
        ...
        <div className="space-y-1 text-[11px] text-slate-800">
          ...
          <Row label="Admission:" value={data.admissionNo || '-'} />
          {data.bedLabel && <Row label="Bed:" value={data.bedLabel} />}
          <Row label="Age/Sex:" value={`${data.age || '-'}/${data.gender || '-'}`} />
          <Row label="Guardian:" value={data.guardian || '-'} />
          <Row label="Dept:" value={data.department || 'IPD'} />
        ...
        <div className="my-3 rounded border border-slate-800 p-3 text-center">
          <div className="text-[10px] font-bold uppercase tracking-widest text-slate-500">{data.isAdvanceReturn ? 'Returned Amount' : data.isAdvance ? 'Advance Amount' : 'Paid Amount'}</div>
          <div className={`text-2xl font-extrabold tracking-widest ${data.isAdvanceReturn ? 'text-rose-600' : ''}`}>Rs {Number(data.payment.amount || 0).toFixed(0)}</div>
        </div>
      ...
      <style>{`
        ...
        #hospital-ipd-payment-receipt { font-family: 'Poppins', Arial, sans-serif }
        @media print {
          ...
          #hospital-ipd-payment-receipt, #hospital-ipd-payment-receipt * { visibility: visible !important }
          #hospital-ipd-payment-receipt { position: absolute !important; left: 0; right: 0; top: 0; margin: 0 auto !important; padding: 10px 10px 0 10px !important; width: 300px !important; box-sizing: border-box !important; line-height: 1.45; overflow: visible !important; z-index: 2147483647; font-size: 14px !important; background: #fff !important }
          #hospital-ipd-payment-receipt, #hospital-ipd-payment-receipt * { color: #000 !important; background: transparent !important; background-color: transparent !important; border-color: #000 !important }
          .print\\:hidden { display: none !important }
          #hospital-ipd-payment-receipt .text-xs{ font-size: 13px !important }
          #hospital-ipd-payment-receipt .text-sm{ font-size: 14px !important }
          #hospital-ipd-payment-receipt .text-lg{ font-size: 18px !important }
          #hospital-ipd-payment-receipt .text-xl{ font-size: 20px !important }
          #hospital-ipd-payment-receipt .row-value{ max-width: none !important; word-break: normal !important; white-space: nowrap !important; text-align: left !important; overflow: hidden !important; text-overflow: ellipsis !important; }
          hr { border-color: #000 !important }
          #hospital-ipd-payment-receipt .grid-cols-\\[100px_1fr\\] { grid-template-columns: 100px 1fr !important; }
          #hospital-ipd-payment-receipt .grid-cols-\\[70px_1fr\\] { grid-template-columns: 70px 1fr !important; }
        }
      `}</style>
    </div>
  )
