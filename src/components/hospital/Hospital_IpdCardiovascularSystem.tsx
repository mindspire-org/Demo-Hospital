...
import CardiovascularExam from './Hospital_CardiovascularExam'

export default function Hospital_IpdCardiovascularSystem({ encounterId }: { encounterId: string }){
  ...
  async function reload(){
    try{
      const res = await hospitalApi.listIpdClinicalNotes(encounterId, { type: 'cardiovascular-exam', limit: 200 }) as any
        ...
        <div className="space-y-4">
          {records.map(r => (
            <div key={r.id} className="rounded-lg border border-slate-200 p-4">
              <CardiovascularExamDisplay data={r.data} />
...
function CardiovascularExamDisplay({ data }: { data: any }) {
  const cvs = data || {}
