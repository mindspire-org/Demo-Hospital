...
export default function Doctor_Patients() {
  ...
  async function load(){
    try {
      ...
      const params: any = { doctorId: doc.id, page: p, limit: l }
      if (f) params.from = f
      if (t) params.to = t
      const [tokRes, presRes] = await Promise.all([
        hospitalApi.listTokens(params) as any,
        hospitalApi.listPrescriptions({ doctorId: doc.id, from: f, to: t, page: 1, limit: 200 }) as any,
      ])
            ...
            >
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                {t.status === 'queued' && (
                  <button
                    className="rounded-md border border-slate-300 px-2 py-1 hover:bg-slate-50"
                    onClick={async (e)=>{ e.stopPropagation(); try { await hospitalApi.updateTokenStatus(t._id, 'in-progress'); invalidateCache('/hospital/tokens'); await load() } catch {} }}
                ...
                {t.status === 'in-progress' && (
                  <button
                    className="rounded-md border border-slate-300 px-2 py-1 hover:bg-slate-50"
                    onClick={async (e)=>{ e.stopPropagation(); try { await hospitalApi.updateTokenStatus(t._id, 'completed'); invalidateCache('/hospital/tokens'); await load() } catch {} }}
                    title="Mark as Complete"
                  >Complete</button>
                )}
                <button
                  className="rounded-md border border-slate-300 px-2 py-1 hover:bg-slate-50 text-amber-700"
                  onClick={async (e)=>{ e.stopPropagation(); try { await hospitalApi.updateTokenStatus(t._id, t.status==='returned'?'queued':'returned'); invalidateCache('/hospital/tokens'); await load() } catch {} }}
                  title="Return token"
