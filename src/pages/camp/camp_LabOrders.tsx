...
import { FlaskConical, Search, Plus, Save } from 'lucide-react'

export default function Camp_LabOrders() {
  ...
  const addOrder = async () => {
    ...
    setError('')
    try {
      const orders = [...(selected.labOrders || []), { testName, status: 'pending', result: '', labOrderId: '' }]
      await campApi.updatePatient(selected._id, { labOrders: orders })
      setTestName('')
      load()
  ...
  const updateResult = async (index: number) => {
    ...
    setError('')
    try {
      const orders = [...(selected.labOrders || [])]
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><FlaskConical className="h-6 w-6 text-emerald-600" /> Lab Orders</h1>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          {!selected ? <div className="py-20 text-center text-slate-400">Select a patient to manage lab orders</div> : (
            <>
              ...
              <div className="flex gap-2">
                <input value={testName} onChange={e => setTestName(e.target.value)} placeholder="Test name" className="flex-1 rounded-xl border border-slate-200 px-4 py-2 text-sm outline-none focus:border-emerald-500" />
              ...
              <div className="space-y-2">
                {(selected.labOrders || []).map((o: any, i: number) => (
                ...
                {(!selected.labOrders || selected.labOrders.length === 0) && <div className="text-sm text-slate-400 py-4 text-center">No lab orders yet</div>}
              </div>
            </>
          )}
        </div>
