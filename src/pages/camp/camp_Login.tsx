...
export default function Camp_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const data: any = await campApi.login(uname, pwd)
      if (!data?.token) {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-emerald-900 via-green-900 to-slate-900 flex items-center justify-center p-4 overflow-hidden relative">
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-48 -right-48 h-96 w-96 rounded-full bg-emerald-500/20 blur-3xl animate-pulse" style={{ animationDuration: '4s' }} />
        <div className="absolute -bottom-56 -left-56 h-112 w-md rounded-full bg-green-500/20 blur-3xl animate-pulse" style={{ animationDuration: '6s', animationDelay: '1s' }} />
        <div className="absolute left-1/2 top-1/2 h-136 w-136 -translate-x-1/2 -translate-y-1/2 rounded-full bg-lime-500/15 blur-3xl animate-pulse" style={{ animationDuration: '5s', animationDelay: '2s' }} />
      </div>
      ...
      <div className="card-3d relative w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-2xl">
        ...
        <div className="relative">
          <div className="relative p-8 pb-6 text-center">
            <div className="mx-auto mb-6 h-24 w-24 overflow-hidden rounded-3xl bg-linear-to-br from-emerald-400/20 to-green-600/20 p-1 backdrop-blur-sm" style={{ animation: 'rotate3d 20s linear infinite' }}>
              <div className="h-full w-full overflow-hidden rounded-[1.3rem] bg-white/10 ring-1 ring-white/20">
                <img src={logoSrc} alt="Medical Camp Portal" className="h-full w-full object-cover" />
              </div>
            </div>
            <h1 className="text-3xl font-black bg-linear-to-r from-emerald-200 via-green-200 to-lime-200 bg-clip-text text-transparent mb-2">Medical Camp</h1>
            <p className="text-sm text-white/60 font-medium">Camp Management Portal</p>
          </div>
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              <div>
                ...
                <div className="relative group">
                  <div className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-emerald-300/60 transition-colors group-focus-within:text-emerald-300">
                  ...
                  <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 backdrop-blur-sm px-12 py-3.5 text-white placeholder-white/40 outline-none transition-all focus:border-emerald-400/50 focus:bg-white/10 focus:ring-4 focus:ring-emerald-400/20" placeholder="Enter your username" autoComplete="username" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative group">
                  <div className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-green-300/60 transition-colors group-focus-within:text-green-300">
                  ...
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 backdrop-blur-sm px-12 py-3.5 pr-14 text-white placeholder-white/40 outline-none transition-all focus:border-green-400/50 focus:bg-white/10 focus:ring-4 focus:ring-green-400/20" placeholder="Enter your password" autoComplete="current-password" />
              ...
              <button type="submit" disabled={loading} className="group relative mt-2 w-full overflow-hidden rounded-2xl bg-linear-to-r from-emerald-500 via-green-500 to-lime-600 px-4 py-4 font-bold text-white shadow-2xl shadow-emerald-500/30 transition-all hover:shadow-emerald-500/50 hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-emerald-400/50 disabled:opacity-70 disabled:cursor-not-allowed">
                <div className="absolute inset-0 bg-linear-to-r from-emerald-400 via-green-400 to-lime-500 opacity-0 transition-opacity group-hover:opacity-100" />
              ...
              <button type="button" onClick={() => navigate('/')} className="mt-3 w-full rounded-2xl border-2 border-white/20 bg-white/5 px-4 py-3 font-semibold text-white/90 hover:text-white hover:bg-white/10 focus:outline-none focus:ring-4 focus:ring-emerald-400/30 transition-colors flex items-center justify-center gap-2">
          ...
          <div className="border-t border-white/10 px-8 py-4 text-center">
            <p className="text-xs text-white/40">Medical Camp Portal</p>
          </div>
        </div>
      </div>
    </div>
