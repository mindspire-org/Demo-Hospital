...
function buildReportHtml({
  ...
  <html>
    ...
    <body>
      ...
      ${table(
        `Lab Tokens (${(labTokens || []).length})`,
        ['Date/Time', 'Token', 'MRN', 'Patient', 'Subtotal', 'Discount', 'Net', 'Received', 'Pending', 'Status', 'Performed By', 'Portal'],
        (labTokens || []).map((t: any) => [
          fmtDateTime12(t.createdAt || new Date().toISOString()),
          String(t.tokenNo || '-'),
          String(t.patient?.mrn || '-'),
          String(t.patient?.fullName || '-'),
          money(t.subtotal || 0),
          money(t.discount || 0),
          money(t.net || 0),
          money(t.receivedAmount || 0),
          money(t.receivableAmount || 0),
          String(t.status || '-'),
          String(t.createdByUsername || t.generatedBy || '-'),
          String(t.portal || 'reception'),
        ])
      )}
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <Section title={`Lab Tokens (${labTokens.length})`} icon={FlaskConical}>
          <SimpleTable
            head={['Date/Time', 'Token', 'MRN', 'Patient', 'Subtotal', 'Discount', 'Net', 'Received', 'Pending', 'Status', 'Performed By', 'Portal']}
            rows={labTokens.map((t: any) => [
              fmtDateTime12(t.createdAt || new Date().toISOString()),
              String(t.tokenNo || '-'),
              String(t.patient?.mrn || '-'),
              String(t.patient?.fullName || '-'),
              currency(Number(t.subtotal || 0)),
              currency(Number(t.discount || 0)),
              currency(Number(t.net || 0)),
              currency(Number(t.receivedAmount || 0)),
              currency(Number(t.receivableAmount || 0)),
              String(t.status || '-'),
              String(t.createdByUsername || t.generatedBy || '-'),
              String(t.portal || 'reception'),
            ])}
