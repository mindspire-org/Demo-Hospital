...
export default function Reception_ERBilling(){
  ...
  async function search(){
    ...
    try{
      ...
      // Filter tokens to only show those that have ER encounters with billing charges
      const tokensWithEncounters = rows.filter(t => t.encounterId)
      
      // Check billing charges in parallel for better performance
      const billingChecks = await Promise.allSettled(
        tokensWithEncounters.map(async (token) => {
          try {
            const chargeRes: any = await hospitalApi.erListBillingItems(token.encounterId, { limit: 1 })
            const hasCharges = chargeRes?.items && chargeRes.items.length > 0
            return hasCharges ? token : null
          } catch {
            return null
          }
        })
