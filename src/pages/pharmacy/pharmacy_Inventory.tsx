...
export default function Pharmacy_Inventory() {
  ...
  const tabs = ['All Items','Pending Review','Low Stock','Expiring Soon','Dead Items','Out of Stock'] as const
  ...
  const [stats, setStats] = useState<{ stockSaleValue: number; lowStockCount: number; outOfStockCount: number; expiringSoonCount: number; deadCount: number }>({ stockSaleValue: 0, lowStockCount: 0, outOfStockCount: 0, expiringSoonCount: 0, deadCount: 0 })
      ...
      try {
        ...
        if (activeTab === 'Pending Review') {
          ...
          const mapped = (items || []).map((it: any) => ({
            ...
            expiry: it.expiry || '-',
            expiryAlertDate: it.expiryAlertDate || undefined,
          ...
          const mapped = (items || []).map((it: any)=>({
            ...
            expiryAlertDate: it.expiryAlertDate || it.earliestExpiryAlert || undefined,
            shelfNumber: it.shelfNumber || '-',
          ...
          // Derived tabs: Low Stock, Expiring Soon, Dead Items, Out of Stock — server-side filtered + paginated
          const status = activeTab === 'Low Stock' ? 'low' : (activeTab === 'Out of Stock' ? 'out' : (activeTab === 'Dead Items' ? 'dead' : 'expiring'))
          ...
          const mapped = (items || []).map((it: any)=>({
            ...
            totalItems: it.onHand ?? 0,
            minStock: (it.minStock != null) ? it.minStock : '-',
            expiry: (status === 'expiring' || status === 'dead' ? String(it.earliestExpiry || '').slice(0,10) : String(it.lastExpiry || it.earliestExpiry || '').slice(0,10)) || '-',
            expiryAlertDate: it.earliestExpiryAlert || it.expiryAlertDate || undefined,
            shelfNumber: it.shelfNumber || '-',
            supplier: it.lastSupplier || '-',
          }))
          setRows(mapped)
          ...
          if (activeTab === 'Expiring Soon') setStats(prev => ({ ...(prev||{ stockSaleValue:0, lowStockCount:0, outOfStockCount:0, expiringSoonCount:0, deadCount:0 }), expiringSoonCount: total }))
          if (activeTab === 'Low Stock') setStats(prev => ({ ...(prev||{ stockSaleValue:0, lowStockCount:0, outOfStockCount:0, expiringSoonCount:0, deadCount:0 }), lowStockCount: total }))
          if (activeTab === 'Out of Stock') setStats(prev => ({ ...(prev||{ stockSaleValue:0, lowStockCount:0, outOfStockCount:0, expiringSoonCount:0, deadCount:0 }), outOfStockCount: total }))
          if (activeTab === 'Dead Items') setStats(prev => ({ ...(prev||{ stockSaleValue:0, lowStockCount:0, outOfStockCount:0, expiringSoonCount:0, deadCount:0 }), deadCount: total }))
        }
      } catch {
        setRows([])
      }
  ...
  function handleExport(){
    ...
    const headers = ['Invoice #','Medicine','Category','Packs','Units/Pack','Unit Sale','Total Items','Min Stock','Shelf','Expiry','Supplier']
    csvRows.push(headers.join(','))
    rows.forEach((r: any)=>{
      const vals = [r.invoice, r.medicine, r.category, r.packs, r.unitsPerPack, r.unitSale, r.totalItems, r.minStock, r.shelfNumber, r.expiry, r.supplier]
  ...
  function handlePrint(){
    const cols = ['Invoice #','Medicine','Generic','Category','Packs','Units/Pack','Unit Sale','Total Items','Min Stock','Shelf','Expiry','Supplier']
    const htmlRows = rows.map((r: any)=>{
      const cells = [r.invoice, r.medicine, r.generic||'-', r.category, r.packs, r.unitsPerPack, r.unitSale, r.totalItems, r.minStock, r.shelfNumber||'-', r.expiry, r.supplier]
