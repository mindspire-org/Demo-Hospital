    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-100 text-violet-700 shadow-sm">
              <Building2 className="h-6 w-6" />
            </div>
            <h2 className="text-2xl font-bold text-slate-800">Departments</h2>
          </div>
          <p className="mt-1 text-sm font-medium text-slate-400 uppercase tracking-wider">{total} DEPARTMENTS REGISTERED</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-violet-200 hover:bg-violet-700 hover:shadow-violet-300 transition-all active:scale-95 uppercase tracking-wide">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100 mb-6">
        <div className="flex items-center gap-2 mb-4 text-xs font-bold text-slate-400 uppercase tracking-widest">
          ...
          <div className="ml-auto flex items-center gap-2">
             <button onClick={exportCSV} className="flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-1.5 text-[11px] font-bold text-emerald-700 hover:bg-emerald-100 transition-colors uppercase">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full rounded-xl border-slate-200 bg-slate-50/50 py-2.5 pl-3 pr-10 text-sm focus:border-violet-500 focus:ring-violet-200 transition-all" 
              />
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-xl border-slate-200 bg-slate-50/50 py-2.5 pl-10 pr-4 text-sm focus:border-violet-500 focus:ring-violet-200 transition-all"
              />
      ...
      {loading ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map(dep => (
            <div key={dep.id} className="group relative rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all hover:shadow-md hover:border-violet-100">
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800 line-clamp-1">{dep.name}</h3>
                    <div className="flex items-center gap-1.5 mt-0.5 text-[11px] font-medium text-slate-400 uppercase">
              ...
              <div className="mt-6 flex items-center justify-between border-t border-slate-50 pt-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-tight">Base Fee:</span>
                  <span className="text-sm font-bold text-emerald-600">Rs {dep.baseFee || 0}</span>
                </div>
                
                <div className="flex gap-2">
                  <button 
                    onClick={()=>openEdit(dep.id)} 
                    className="text-[10px] font-bold uppercase tracking-wider text-violet-600 hover:bg-violet-50 px-2 py-1 rounded transition-colors"
                  >
                    Edit
                  </button>
                  <button 
                    onClick={()=>setDeleteId(dep.id)} 
                    className="text-[10px] font-bold uppercase tracking-wider text-rose-600 hover:bg-rose-50 px-2 py-1 rounded transition-colors"
                  >
                    Delete
                  </button>
                </div>
              ...
              {dep.description && (
                <div className="mt-3 text-xs text-slate-500 line-clamp-2 leading-relaxed italic">
                  "{dep.description}"
                </div>
              )}
            </div>
