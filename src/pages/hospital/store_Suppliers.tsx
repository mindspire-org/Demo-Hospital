...
export default function Store_Suppliers() {
  ...
  // Use refs to avoid stale closure issues and prevent duplicate calls
  const loadingRef = useRef(false)
  const cancelledRef = useRef(false)
  const prevDepsRef = useRef({ statusFilter, query })
  ...
  const loadItems = async (p = 1) => {
    ...
    setLoading(true)
    try {
      const res = await hospitalApi.listStoreSuppliers({ status: statusFilter || undefined, search: query || undefined, page: p, limit }) as any
  ...
  // Single effect with debounce for all data loading
  useEffect(() => {
    cancelledRef.current = false
    prevDepsRef.current = { statusFilter, query }
    
    // Debounce for all changes to prevent rapid re-fetching
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        <select value={limit} onChange={e => { setLimit(Number(e.target.value)); setPage(1); }} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
      ...
      <div className="mt-5 overflow-x-auto">
            ...
            {loading ? (
              ...
              <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="px-3 py-2">
                  <button onClick={() => setLedgerSupplierId(s.id)} className="font-medium text-sky-700 hover:underline text-left">{s.name}</button>
                </td>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-slate-500">Showing {items.length} of {total}</span>
        </div>
      </div>
