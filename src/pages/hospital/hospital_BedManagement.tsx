          ...
          <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
            {group.items.map(b => (
              <div
                ...
                onClick={() => {
                  if (b.status==='occupied' && b.occupantEncounterId) {
                    // Navigate to appropriate profile based on encounter type
                    if (b.occupantEncounterType === 'ER') {
                      navigate(`/hospital/emergency/${b.occupantEncounterId}`)
                    } else {
                      navigate(`/hospital/patient/${b.occupantEncounterId}`)
                    }
                  }
                }}
              ...
              >
                ...
                {b.status === 'occupied' && (
                  <button
                    className="mt-2 w-full rounded-md bg-rose-100 px-2 py-1 text-xs font-medium text-rose-700 hover:bg-rose-200 transition-colors"
