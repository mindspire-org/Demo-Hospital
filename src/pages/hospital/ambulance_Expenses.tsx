...
export default function Ambulance_Expenses() {
  ...
  const loadExpenses = async (p = 1) => {
    ...
    try {
      ...
      if (!cancelledRef.current) setExpenses([])
    } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (!cancelledRef.current) loadExpenses(1)
    }, 100)
    return () => {
      cancelledRef.current = true
      clearTimeout(timer)
    }
  }, [ambulanceFilter, categoryFilter, from, to, query])
  ...
  const saveAdd = async () => {
    ...
    try {
      ...
      loadExpenses(page)
      setShowAdd(false)
      setAddForm({ ambulanceId: '', category: 'Fuel', amount: '', date: getLocalDate(), description: '', receiptNo: '' })
  ...
  const exportCSV = () => {
    ...
    a.href = url
    a.download = `expenses-report-${getLocalDate()}.csv`
    a.click()
