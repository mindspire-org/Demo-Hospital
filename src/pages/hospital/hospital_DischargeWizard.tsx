    ...
    try {
      ...
      if (enc && enc._id){
        ...
        setPatient({
          ...
          department: enc.departmentId?.name||'',
          admissionNo: enc.admissionNo || '',
          encounterType: 'IPD',
          guardian: enc.patientId?.guardianRel || '',
          guardianName: enc.patientId?.fatherName || '',
          fatherName: enc.patientId?.fatherName || '',
        })
        setLoading(false)
        return
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="text-lg font-semibold text-slate-800">IPD Discharge Forms</div>
        <div className="text-sm text-slate-600">Generate discharge documents and certificates for this patient.</div>
      </div>

      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap gap-2">
            {['1. Select Forms','2. Fill & Print'].map((t, i)=> (
              <button key={i} onClick={()=>setStep(i)} className={`rounded-md px-3 py-1 text-sm ${step===i?'bg-navy text-white':'bg-slate-100 text-slate-700'}`}>{t}</button>
            ))}
          </div>
          {patient.id && (
            <button onClick={() => {
              if (returnTo) {
                ...
                navigate(`/hospital/search-patients?id=${patient.id}`)
              }
      ...
      {step===1 && (
        <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
          <div className="text-lg font-semibold text-slate-800">Fill & Print Forms</div>
