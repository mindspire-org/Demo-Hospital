...
export default function Hospital_IPDServices(){
  ...
  async function reload(){
    setLoading(true)
    try{
      const res: any = await hospitalApi.listIpdServices({
        q: q.trim() || undefined,
  ...
  const confirmDeleteService = async () => {
    ...
    if (!id) return
    try{
      await hospitalApi.deleteIpdService(id)
  ...
  return (
    ...
    <div className="p-4 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-sm text-slate-600">In-Patient Department</div>
          <h2 className="text-xl font-semibold text-slate-800">IPD Services & Prices</h2>
          <div className="mt-1 text-xs text-slate-500">Manage IPD service catalog</div>
        </div>
                    ...
                    <tr key={s.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          onChange={async (e)=>{
                            try{
                              await hospitalApi.updateIpdService(s.id, { active: e.target.checked })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form
            onSubmit={async (e)=>{
              e.preventDefault()
              try{
                await hospitalApi.updateIpdService(editDraft.id, {
