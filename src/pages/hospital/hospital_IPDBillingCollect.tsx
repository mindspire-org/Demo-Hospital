...
export default function Reception_IPDBillingCollect(){
  ...
  useEffect(() => {
    encIdRef.current = encId
    qRef.current = q
    fromDateRef.current = fromDate
    toDateRef.current = toDate
  }, [encId, q, fromDate, toDate])
  ...
  async function search(){
    ...
    try{
      ...
      let rows = (res.admissions||[]).map((a:any)=>({
        id: String(a._id),
      ...
      if (fDate && tDate) {
        ...
        rows = rows.filter((r: any) => {
          const admitDate = new Date(r.startAt || 0)
  ...
  async function collect(){
    ...
    try{
      ...
      if (amt > 0){
        ...
        await hospitalApi.createIpdPayment(encId, { amount: amt, method, paymentMode: method, refNo, receivedBy: getReceptionUser(), portal, allocations } as any)
      }
      // Reload fresh data
      const [e, bi, pay] = await Promise.all([
