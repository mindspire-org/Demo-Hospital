...
export default function Ambulance_Fuel() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (!cancelledRef.current) loadRecords(1)
    }, 100)
    return () => {
      cancelledRef.current = true
      clearTimeout(timer)
    }
  }, [ambulanceFilter, from, to, query])
  ...
  const saveAdd = async () => {
    ...
    try {
      ...
      loadRecords(page)
      setShowAdd(false)
      setAddForm({ ambulanceId: '', date: getLocalDate(), quantity: '', cost: '', station: '', odometer: '', receiptNo: '', notes: '' })
  ...
  const exportCSV = () => {
    ...
    a.href = url
    a.download = `fuel-report-${getLocalDate()}.csv`
    a.click()
