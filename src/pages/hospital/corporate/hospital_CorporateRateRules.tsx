...
let cacheCompanies: Array<{ id: string; name: string }> | null = null
let cacheDoctors: Array<{ id: string; name: string; fee?: number }> | null = null
let cacheDepartments: Array<{ id: string; name: string; baseFee?: number }> | null = null
let cacheLabTests: Array<{ id: string; name: string; price?: number }> | null = null
let cacheDiagTests: Array<{ id: string; name: string; price?: number }> | null = null
...
export default function Hospital_CorporateRateRules(){
  ...
  const [filters, setFilters] = useState<{ companyId: string; scope: typeof SCOPES[number] | '' }>(() => {
    try {
      const raw = sessionStorage.getItem('hospital.corpRateRules.state')
      if (raw) {
        const s = JSON.parse(raw) as any
        const scope = String(s?.scope || '')
        return {
          companyId: String(s?.companyId || ''),
          scope: (SCOPES as readonly string[]).includes(scope) ? (scope as any) : '',
        }
      }
    } catch {}
  ...
  const [page, setPage] = useState(() => {
    try {
      ...
      if (raw) {
        const s = JSON.parse(raw) as any
        const v = Number(s?.page || 1)
        return isFinite(v) && v > 0 ? Math.floor(v) : 1
      }
    } catch {}
    return 1
  })
  ...
  const [unpagedRows, setUnpagedRows] = useState<any[] | null>(null)
  const [unpagedKey, setUnpagedKey] = useState<string>('')
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('hospital.corpRateRules.state', JSON.stringify({ companyId: filters.companyId, scope: filters.scope, page, limit }))
    } catch {}
  }, [filters.companyId, filters.scope, page, limit])

  useEffect(()=>{
    let mounted = true
    ;(async()=>{
      try {
        if (cacheCompanies) setCompanies(cacheCompanies)
        if (cacheDoctors) setDoctors(cacheDoctors)
        if (cacheDepartments) setDepartments(cacheDepartments)
        if (cacheLabTests) setLabTests(cacheLabTests)
        if (cacheDiagTests) setDiagTests(cacheDiagTests)

        if (!cacheCompanies || !cacheDoctors || !cacheDepartments){
          ...
          const doctorArr = ((d?.doctors)||[]).map((x:any)=>({ id: String(x._id||x.id), name: x.name, fee: Number(x.opdBaseFee||0) }))
          const depArrRaw: any[] = (deps?.departments || deps?.data || []) as any[]
          const depArr = (depArrRaw||[]).map((x:any)=>({ id: String(x._id||x.id), name: x.name, baseFee: Number(x.opdBaseFee||0) }))

          cacheCompanies = companyArr
          cacheDoctors = doctorArr
          cacheDepartments = depArr
  ...
  const loadingLabTests = useRef(false)
  const loadingDiagTests = useRef(false)

  async function ensureLabTests(){
    if (cacheLabTests){
      setLabTests(cacheLabTests)
      return
    }
    if (loadingLabTests.current) return
    loadingLabTests.current = true
    try {
      const lt = await labApi.listTests({ limit: 1000 }) as any
      const arr = ((lt?.items)||[]).map((x:any)=>({ id: String(x._id||x.id), name: x.name, price: Number(x.price||0) }))
      cacheLabTests = arr
      setLabTests(arr)
    } finally {
      loadingLabTests.current = false
    }
  }

  async function ensureDiagTests(){
    if (cacheDiagTests){
      setDiagTests(cacheDiagTests)
      return
    }
    if (loadingDiagTests.current) return
    loadingDiagTests.current = true
    try {
      const dt = await diagnosticApi.listTests({ limit: 1000 }) as any
      const arr = ((dt?.items)||[]).map((x:any)=>({ id: String(x._id||x.id), name: x.name, price: Number(x.price||0) }))
      cacheDiagTests = arr
      setDiagTests(arr)
    } finally {
      loadingDiagTests.current = false
    }
  }
  ...
  async function load(){
    setLoading(true)
    try {
      const key = `${filters.companyId || ''}|${filters.scope || ''}`

      if (unpagedRows && unpagedKey === key){
        const start = (page - 1) * limit
        const sliced = unpagedRows.slice(start, start + limit)
        setRows(sliced)
        setTotal(unpagedRows.length)
        setLoading(false)
        return
      }
      ...
      const items = (res?.rules || res?.items || []) as any[]
      const t = (res?.total ?? res?.count ?? res?.totalCount)
      const needsLab = items.some(r => String(r?.scope || '').toUpperCase() === 'LAB')
      const needsDiag = items.some(r => String(r?.scope || '').toUpperCase() === 'DIAG')
      ...
      if (typeof t === 'number'){
        ...
        setUnpagedRows(null)
        setUnpagedKey('')
        if (needsLab) ensureLabTests()
        if (needsDiag) ensureDiagTests()
      } else if (items.length > limit){
        setUnpagedRows(items)
        setUnpagedKey(key)
        setTotal(items.length)
        const start = (page - 1) * limit
        setRows(items.slice(start, start + limit))
        if (needsLab) ensureLabTests()
        if (needsDiag) ensureDiagTests()
      } else {
        setRows(items)
  ...
  useEffect(() => {
    setUnpagedRows(null)
    setUnpagedKey('')
  }, [filters.companyId, filters.scope, limit])
  ...
  useEffect(() => {
    if (!showAdd) return
    if (createForm.scope === 'LAB') ensureLabTests()
    if (createForm.scope === 'DIAG') ensureDiagTests()
  }, [showAdd, createForm.scope])
