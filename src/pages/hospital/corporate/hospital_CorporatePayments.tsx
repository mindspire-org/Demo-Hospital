...
export default function Hospital_CorporatePayments(){
  ...
  const [paySort, setPaySort] = useState<string>('dateIso')
  const [payOrder, setPayOrder] = useState<'asc'|'desc'>('desc')
  const [outSort, setOutSort] = useState<string>('dateIso')
  const [outOrder, setOutOrder] = useState<'asc'|'desc'>('desc')
  ...
  function handlePaySort(field: string){
    if (paySort === field) {
      setPayOrder(payOrder === 'asc' ? 'desc' : 'asc')
    } else {
      setPaySort(field)
      setPayOrder('asc')
    }
  ...
  function PaySortHeader({ field, children, align = 'left' }: { field: string; children: React.ReactNode; align?: 'left'|'right' }) {
    const active = paySort === field
    return (
      <th
        className={`px-2 py-2 cursor-pointer hover:bg-slate-100 ${align === 'right' ? 'text-right' : 'text-left'}`}
        onClick={() => handlePaySort(field)}
      >
        <span className="flex items-center gap-1">
          {children}
          {active && (
            <span className="text-xs">{payOrder === 'asc' ? '↑' : '↓'}</span>
          )}
        </span>
      </th>
    )
  ...
  function handleOutSort(field: string){
    if (outSort === field) {
      setOutOrder(outOrder === 'asc' ? 'desc' : 'asc')
    } else {
      setOutSort(field)
      setOutOrder('asc')
    }
  ...
  function OutSortHeader({ field, children, align = 'left' }: { field: string; children: React.ReactNode; align?: 'left'|'right' }) {
    const active = outSort === field
    return (
      <th
        className={`px-2 py-2 cursor-pointer hover:bg-slate-100 ${align === 'right' ? 'text-right' : 'text-left'}`}
        onClick={() => handleOutSort(field)}
      >
        <span className="flex items-center gap-1">
          {children}
          {active && (
            <span className="text-xs">{outOrder === 'asc' ? '↑' : '↓'}</span>
          )}
        </span>
      </th>
    )
  ...
  useEffect(()=>{ loadPayments() }, [companyId, payPage, payLimit, paySort, payOrder])
  ...
  useEffect(()=>{ setTxPage(1); setPayPage(1) }, [companyId])
  ...
  const sortedOutstanding = useMemo(()=> {
    const arr = [...(txRows||[])]
    const f = outSort
    return arr.sort((a:any,b:any)=> outOrder === 'asc' ? cmpField(a,b,f) : -cmpField(a,b,f))
  }, [txRows, outSort, outOrder])
  const sortedPayRows = useMemo(()=> {
    const arr = [...(payRows||[])]
    const f = paySort
    return arr.sort((a:any,b:any)=> {
      const c = cmpField(a,b,f)
      return payOrder === 'asc' ? c : -c
    })
  }, [payRows, paySort, payOrder])
