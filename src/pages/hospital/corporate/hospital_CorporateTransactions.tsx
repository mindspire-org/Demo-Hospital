...
export default function Hospital_CorporateTransactions(){
  ...
  const [sort, setSort] = useState<string>('dateIso')
  ...
  function valueFor(o: any, f: string){
    if (f === 'dateIso') return o?.dateIso || o?.createdAt || ''
    if (f === 'companyId') return o?.companyName || o?.corporateName || companyNameMap[String(o?.companyId || o?.corporateId || o?.panelId || '')] || ''
    return o?.[f]
  }
  function cmpValues(a: any, b: any, f: string){
    const va = valueFor(a, f)
    const vb = valueFor(b, f)
    if (f === 'patientMrn' || /mrn/i.test(f)){
      const sa = String(va || '')
      const sb = String(vb || '')
      const pa = sa.match(/\d+/)?.[0]
      const pb = sb.match(/\d+/)?.[0]
      const na = pa != null ? Number(pa) : NaN
      const nb = pb != null ? Number(pb) : NaN
      if (!isNaN(na) && !isNaN(nb) && sa.replace(/\d+/g,'') === sb.replace(/\d+/g,'')){
        return na - nb
      }
