...
export default function Hospital_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Also store as lab.token since Hospital pages use Lab APIs for patient data
      try {
        if (res?.token) {
      ...
      let redirectTo = '/hospital/token-generator'
      try {
        const saved = sessionStorage.getItem('hospital.redirect')
        if (saved && saved.startsWith('/hospital') && !saved.includes('/login')) {
          redirectTo = saved
          sessionStorage.removeItem('hospital.redirect')
        }
      } catch {}
      navigate(redirectTo)
      return
