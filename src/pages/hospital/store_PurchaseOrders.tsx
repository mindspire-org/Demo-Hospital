...
export default function Store_PurchaseOrders() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      setError('Failed to load purchase orders. Please check if the backend is running.');
      setOrders([]);
      setTotal(0);
      setTotalPages(1);
    } finally {
      ...
      <body>
        <div class="header">
          <div class="pharmacy-info">
            ...
            <h1>${settings?.name || settings?.hospitalName || settings?.pharmacyName || 'Hospital'}</h1>
        ...
        <div class="info-grid">
          <div class="info-box">
            ...
            <p><strong>Company:</strong> ${order.supplierCompany || order.companyName || 'N/A'}</p>
            <p><strong>Phone:</strong> ${order.supplierPhone || 'N/A'}</p>
            <p><strong>Address:</strong> ${order.supplierAddress || 'N/A'}</p>
            <p><strong>Tax ID:</strong> ${order.supplierTaxId || 'N/A'}</p>
          </div>
          <div class="info-box">
            <h3>Company Information</h3>
            <div class="name">${settings?.name || settings?.hospitalName || settings?.pharmacyName || 'Hospital'}</div>
            <p>${settings?.address || 'Hospital Address'}</p>
            <p>Phone: ${settings?.phone || ''}</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Manage store item orders from suppliers</p>
          </div>
        </div>
        <button
