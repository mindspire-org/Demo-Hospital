...
export default function Hospital_IPDServiceAdd(){
  const navigate = useNavigate()
  const [sp] = useSearchParams()

  const returnTo = useMemo(() => {
    const rt = sp.get('returnTo')
    return rt ? String(rt) : '/hospital/ipd-services'
  }, [sp])
    ...
    <div className="p-4 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-xl font-semibold text-slate-800">Add IPD Service</h2>
          <div className="mt-1 text-xs text-slate-500">Create a service and set its price</div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="e.g. ICU Daily Charge"
            />
          </div>
          <div>
