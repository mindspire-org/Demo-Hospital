...
import Lab_AddCompanyDialog, { type Company } from '../../components/lab/lab_AddCompanyDialog'
import { labApi } from '../../utils/api'

export default function Lab_Companies(){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await labApi.listCompanies({ distributorId: query || undefined })
        if (!mounted) return
  ...
  useEffect(() => {
    ...
    window.addEventListener('lab:companies:refresh', onRefresh as any)
    return () => window.removeEventListener('lab:companies:refresh', onRefresh as any)
  }, [])

  const addCompany = async (c: Company) => {
    try {
      await labApi.createCompany({ name: c.name, status: c.status })
      setReloadTick(t=>t+1)
  ...
  const saveEdit = async (c: Company & { id: string }) => {
    try {
      await labApi.updateCompany(c.id, { name: c.name, status: c.status })
      setReloadTick(t=>t+1)
    } catch (e) { console.error(e) }
  }

  const remove = async (id: string) => {
    try { await labApi.deleteCompany(id); setReloadTick(t=>t+1) } catch (e) { console.error(e) }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Dialogs */}
      <Lab_AddCompanyDialog open={addOpen} onClose={()=>setAddOpen(false)} onSave={addCompany} />
      {selected && (
        <Lab_AddCompanyDialog open={editOpen} onClose={()=>{ setEditOpen(false); setSelected(null) }} onSave={(c)=>saveEdit({ ...c, id: selected.id })} initial={{ id: selected.id, name: selected.name, status: (selected.status||'Active') as any }} title="Edit Company" submitLabel="Save" />
      )}
    </div>
  )
}
