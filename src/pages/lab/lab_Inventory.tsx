...
import { RotateCw, FileDown, CalendarDays, Package, TrendingDown, AlertTriangle, Plus } from 'lucide-react'
...
export default function Lab_Inventory() {
  ...
  const fromPending = searchParams.get('tab') === 'pending'
  ...
  const [stats, setStats] = useState<{ stockCostValue: number; lowStockCount: number; outOfStockCount: number; expiringSoonCount: number }>({ stockCostValue: 0, lowStockCount: 0, outOfStockCount: 0, expiringSoonCount: 0 })
  ...
  const exportCsv = () => {
    try {
      const headers = ['Invoice','Item','Category','Packs','Units/Pack','Total Units','Min Stock','Expiry','Supplier']
    ...
    const load = async () => {
      try {
        ...
        try {
          ...
          setStats(prev => ({ ...(prev||{ stockCostValue:0, lowStockCount:0, outOfStockCount:0, expiringSoonCount:0 }), expiringSoonCount: countLast }))
        } catch {}
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button onClick={() => navigate('/lab/inventory/add-invoice' + (fromPending ? '?from=pending' : ''))} className="btn"><Plus className="h-4 w-4" /> Add Invoice</button>
        ...
        <div className="mb-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="card p-4 border-emerald-200 bg-emerald-50/40">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-sm text-slate-600">Stock Cost Value</div>
                <div className="mt-1 text-lg font-semibold text-emerald-700">{stats.stockCostValue?.toFixed ? stats.stockCostValue.toFixed(2) : '0.00'}</div>
              </div>
        ...
        <Lab_InventoryTable
          ...
          onEditDraft={(id) => {
            ...
            navigate(`/lab/inventory/edit-invoice/${encodeURIComponent(id)}${search}`)
          }}
          onDelete={async (row) => {
