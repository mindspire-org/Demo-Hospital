...
export default function Lab_PurchaseOrders() {
  ...
  const loadOrders = async () => {
    try {
      const res: any = await labApi.listPurchaseOrders({ q: query, page, limit });
      setOrders(res.items || []);
  ...
  const handleCreate = async (data: any) => {
    try {
      await labApi.createPurchaseOrder(data);
      loadOrders();
    } catch (error) {
  ...
  const handleUpdate = async (data: any) => {
    try {
      ...
      await labApi.updatePurchaseOrder(selectedOrder._id, data);
      loadOrders();
    } catch (error) {
  ...
  const handleMarkAsSent = async (id: string) => {
    try {
      await labApi.updatePurchaseOrderStatus(id, 'Sent');
      loadOrders();
    } catch (error) {
  ...
  const handleMarkAsComplete = async (id: string) => {
    try {
      await labApi.updatePurchaseOrderStatus(id, 'Complete');
      loadOrders();
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (!toDelete) return;
    try {
      await labApi.deletePurchaseOrder(toDelete);
      loadOrders();
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .lab-info h1 { color: #1e3a8a; margin: 0; font-size: 24px; }
          .lab-info p { margin: 4px 0; color: #666; font-size: 14px; }
      ...
      <body>
        <div class="header">
          <div class="lab-info">
            ...
            <h1>${settings?.labName || settings?.name || 'Lab'}</h1>
        ...
        <div class="info-grid">
          ...
          <div class="info-box">
            ...
            <div class="name">${order.companyName || '-'}</div>
          </div>
        </div>
