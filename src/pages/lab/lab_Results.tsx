...
import { Search, ChevronLeft, Plus, Barcode, Building2, Clock, FlaskConical, User, CalendarDays, FileCheck, Printer, Save, Send, AlertTriangle, X, ChevronRight, ChevronDown, Activity, ListChecks, CheckCircle2, Eye } from 'lucide-react'
...
type LabTest = {
  ...
  parameters?: Array<{ name: string; unit?: string; normalRangeMale?: string; normalRangeFemale?: string; normalRangePediatric?: string; formula?: string; dependencies?: string[]; kind?: 'quantitative'|'qualitative'; criticalMin?: number; criticalMax?: number; sectionKey?: string; qualitativeOptions?: string[]; interpretationRules?: Array<{ expression: string; label?: string; text?: string }>; decimals?: number }>
  template?: string
...
type ResultRow = { id: string; test: string; normal?: string; unit?: string; prevValue?: string; value?: string; flag?: 'normal'|'abnormal'|'abnormal_low'|'abnormal_high'|'critical'|'critical_low'|'critical_high'; comment?: string; formula?: string; dependencies?: string[]; isCalculated?: boolean; lockedMeta?: boolean; kind?: 'quantitative'|'qualitative'; qualitativeOptions?: string[]; criticalMin?: number; criticalMax?: number; sectionKey?: string; interpretationRules?: Array<{ expression: string; label?: string; text?: string }>; decimals?: number }
...
function autoFlagForRow(r: ResultRow): ResultRow['flag'] | undefined {
  ...
  if (r.criticalMin != null && Number.isFinite(r.criticalMin) && num <= r.criticalMin) return 'critical_low'
  if (r.criticalMax != null && Number.isFinite(r.criticalMax) && num >= r.criticalMax) return 'critical_high'
...
export default function Lab_Results() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all'|'pending'|'sample_collected'|'in_progress'|'result_entered'|'approved'|'returned'|'outsourced'|'dispatched'|'completed'>('all')
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try {
        ...
        setTests((testsRes.items||[]).map((t:any)=>({ id: t._id, name: t.name, price: Number(t.price||0), parameter: t.parameter, unit: t.unit, normalRangeMale: t.normalRangeMale, normalRangeFemale: t.normalRangeFemale, normalRangePediatric: t.normalRangePediatric, parameters: Array.isArray(t.parameters)? t.parameters : [], template: t.template || 'general', sections: Array.isArray(t.sections)? t.sections : [] })))
  ...
  const [saveError, setSaveError] = useState<{ code?: string; message: string; validation?: any } | null>(null)
  ...
  const onSelect = async (o: Order, testId: string, orderTestId: string | null = null) => {
    setSelected(o)
    ...
    try {
      ...
      const existing = (res.items || []).find((r: any) =>
        String(r.testId || '') === String(testId || '') ||
        String(r.orderTestId || '') === String(orderTestId || '')
      ) || (res.items || []).find((r: any) => String(r.orderId || '') === String(o.id))
      if (existing) {
        setRows((existing.rows || []).map((r: any) => ({
    ...
    } catch (e) { console.error('Failed to check existing result', e) }
  ...
  const addRow = () => setRows(prev => [...prev, { id: genId(), test: '', normal: '', unit: '', value: '', comment: '', lockedMeta: false }])
  const removeRow = (id: string) => setRows(prev => prev.filter(r => r.id !== id))

  // Calculate derived parameters based on formulas
  const calculateDerived = (rows: ResultRow[]): ResultRow[] => {
    // Build a map of parameter names to values
    const valueMap: Record<string, number> = {}
    for (const r of rows) {
      const key = (r.test || '').trim().toLowerCase().replace(/[^a-z0-9_]/gi, '_')
    ...
    return rows.map(r => {
      ...
      try {
        ...
        for (const [key, val] of Object.entries(valueMap)) {
          ...
          const regex = new RegExp(`\\b${key.replace(/_/g, '[^a-z0-9]?')}\\b`, 'gi')
  ...
  const updateRowValue = (id: string, value: string) => {
    setRows(prev => {
      ...
      return calculateDerived(updated)
    })
  }

  const save = async (opts?: { strict?: boolean }) => {
    ...
    try {
      ...
      if (existingResultId){
        const r: any = await labApi.updateResult(existingResultId, { rows: rowsToSave, interpretation: interpretation.trim() || undefined, testId: selectedTestId || undefined, testName: testName || undefined, strict: opts?.strict === true })
        ...
        const r: any = await labApi.createResult({ orderId: selected.id, orderTestId: selectedOrderTestId || undefined, testId: selectedTestId || undefined, testName: testName || undefined, rows: rowsToSave, interpretation: interpretation.trim() || undefined, submittedBy, strict: opts?.strict === true })
      ...
      if (savedId) {
        try {
          const res: any = await api(`/lab/critical-events?resultId=${savedId}&status=open`)
          const events = (res?.items || []).filter((e: any) => !e.resolvedAt)
          if (events.length) { setPendingCriticalEvents(events); return true }
        } catch {}
      }

      setTick(t=>t+1)
      setToast({ type: 'success', message: `Results ${existingResultId ? 'updated' : 'saved'} successfully for ${testName || 'test'}!` })
      ...
      if (parsed?.code === 'TOTAL_PERCENT_MISMATCH' || /TOTAL_PERCENT_MISMATCH/.test(msg)) {
        setSaveError({
          code: 'TOTAL_PERCENT_MISMATCH',
          message: parsed?.message || 'Differential total is not 100%. Report cannot be submitted — please review entries.',
  ...
  const handleValueKeyDown = useCallback((e: React.KeyboardEvent, rowIdx: number) => {
  ...
  const filteredItems = items.filter(o => !selectedCompany || o.corporateId === selectedCompany)
  ...
  const statusCounts = useMemo(() => {
    ...
    for (const o of filteredItems) {
      const tList = (o.testStatuses && o.testStatuses.length > 0) ? o.testStatuses : o.tests
      for (const t of tList) {
        let s = 'pending'
        if (o.testStatuses && o.testStatuses.length > 0) {
          const tid = typeof t === 'object' && t?.testId ? t.testId : String(t)
          const ts = o.testStatuses.find((x: any) => String(x.testId) === String(tid))
          s = ts?.status || 'pending'
        }
      ...
      <div className="space-y-4 p-4 md:p-6">
        ...
        <div className="rounded-2xl bg-linear-to-r from-violet-600 via-sky-600 to-emerald-500 p-5 text-white shadow-lg shadow-sky-200/50">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-0.5 text-sm text-sky-100">Select a sample to enter test results</div>
            </div>
            <div className="text-sm text-sky-100">{total} sample{total !== 1 ? 's' : ''} pending</div>
          </div>
        </div>

        {/* Mini Dashboard */}
        <MiniDashboard cards={[
          { label: 'Total', value: Object.values(statusCounts).reduce((a,b)=>a+b,0), icon: ListChecks, color: 'bg-sky-500' },
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[220px]">
              ...
              <input value={q} onChange={e => { setQ(e.target.value); setPage(1) }} placeholder="Search by ID, patient, token, CNIC, phone..." className="w-full rounded-lg border border-slate-300 bg-white py-2 pl-9 pr-3 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200" />
            </div>
            <div className="relative min-w-[170px]">
              <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
              <select value={selectedCompany} onChange={e => { setSelectedCompany(e.target.value); setPage(1) }} className="w-full appearance-none rounded-lg border border-slate-300 bg-white py-2 pl-9 pr-8 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200">
                <option value="">All Companies</option>
                {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <select value={rowsPer} onChange={e => { setRowsPer(Number(e.target.value)); setPage(1) }} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1 text-sm">
            {([
              ...
              <button key={val} type="button" onClick={()=>{ setStatusFilter(val as any); setPage(1) }} className={`rounded-md px-3 py-1.5 border text-xs ${statusFilter===val?'bg-violet-600 text-white border-violet-600':'border-slate-300 text-slate-700 hover:bg-slate-50'}`}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredItems.reduce((acc: any[], o) => {
                  ...
                  testsToProcess.forEach((t: any, idx: number) => {
                    const tid = typeof t === 'object' && t?.testId ? t.testId : String(t)
                    const otid = typeof t === 'object' && t?.orderTestId ? t.orderTestId : null
                    let testStatus = 'pending'
                    if (o.testStatuses && o.testStatuses.length > 0) {
                      const ts = o.testStatuses.find((s: any) => String(s.testId) === String(tid))
                      testStatus = ts?.status || 'pending'
                    }
                    if (statusFilter !== 'all' && testStatus !== statusFilter) return
                    const tname = (typeof t === 'object' && t?.testName) ? t.testName : (testsMap[tid]?.name || '—')
                    acc.push(
                      <tr key={`${o.id}-${tid}-${idx}-${otid || idx}`} className="hover:bg-slate-50/60 transition-colors">
                        <td className="px-4 py-3 text-slate-500">{start + acc.length}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-slate-700">{formatDateTime(o.createdAt)}</td>
                        <td className="px-4 py-3">
                          <div className="font-medium text-slate-900">{o.patient.fullName}</div>
                          {o.patient.phone && <div className="text-xs text-slate-500">{o.patient.phone}</div>}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap font-mono text-slate-700">{token}</td>
                        ...
                        <td className="px-4 py-3 text-slate-600">{o.patient.mrn || '-'}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          {o.billingType === 'corporate' ? (
                            <span className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-2.5 py-1 text-xs font-semibold text-violet-700 ring-1 ring-violet-200" title={o.corporateName || 'Corporate'}>
                              <Building2 className="h-3 w-3" />{o.corporateName || 'Corporate'}
                            </span>
                          ) : (
                            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">Cash</span>
                          )}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ring-1 ${
                            testStatus === 'approved' || testStatus === 'completed' ? 'bg-emerald-100 text-emerald-700 ring-emerald-200' :
                            testStatus === 'sample_collected' ? 'bg-blue-100 text-blue-700 ring-blue-200' :
                            testStatus === 'in_progress' ? 'bg-amber-100 text-amber-700 ring-amber-200' :
                            'bg-slate-100 text-slate-600 ring-slate-200'
                          }`}>
                            {testStatus === 'sample_collected' && <Clock className="h-3 w-3" />}
                            {testStatus}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1.5">
                            <button onClick={() => onSelect(o, String(tid), otid)} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold text-white shadow-sm transition-colors ${
                              testStatus === 'result_entered' || testStatus === 'approved' || testStatus === 'completed'
          ...
          <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3 text-sm text-slate-600">
            <div className="text-xs">{total === 0 ? '0' : `${start}–${end}`} of {total}</div>
            <div className="flex items-center gap-2">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={curPage <= 1} className="inline-flex items-center gap-1 rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium disabled:opacity-40 hover:bg-slate-50 transition-colors"><ChevronLeft className="h-3 w-3" /> Prev</button>
              <span className="text-xs font-medium">{curPage} / {pageCount}</span>
              <button onClick={() => setPage(p => Math.min(pageCount, p + 1))} disabled={curPage >= pageCount} className="inline-flex items-center gap-1 rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium disabled:opacity-40 hover:bg-slate-50 transition-colors">Next <ChevronRight className="h-3 w-3" /></button>
            </div>
          </div>
        </div>
  ...
  const selectedTestStatus = selected.testStatuses?.find(ts => String(ts.testId) === String(selectedTestId))?.status
  const isLocked = (selectedTestStatus === 'approved' || selectedTestStatus === 'completed') && !session.isAdmin
    ...
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button onClick={() => { if (returnTo === 'report-approval') { navigate('/lab/report-approval') } else { setSelected(null); setSelectedTestId(null) } }} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">
        ...
        <div className="flex items-center gap-2">
          <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full bg-violet-100 px-3 py-1 text-xs font-bold text-violet-700 ring-1 ring-violet-200">
            <FlaskConical className="h-3 w-3" /> {testName}
          </span>
          <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs font-mono text-slate-600 ring-1 ring-slate-200">
            <Barcode className="h-3 w-3" /> {selected.barcode || genBarcode(selected)}
          </span>
        </div>
      </div>
      ...
      {isLocked && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-medium text-amber-800 flex items-center gap-2">
          ...
          This result is <strong>{selectedTestStatus}</strong> and locked for editing. Only admins can override.
        </div>
      )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-linear-to-r from-slate-50 to-white p-4">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-base font-bold text-slate-900">{patient.fullName}</div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-slate-600">{formatDateTime(selected.createdAt)}</span>
            </div>
            <div className="flex items-center gap-2">
              <label className="text-xs font-semibold text-slate-600">Ref. Doctor</label>
              <input value={referring} onChange={e => setReferring(e.target.value)} className="w-52 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200" placeholder="Optional" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 rounded-lg border border-violet-200 bg-violet-50 px-4 py-2 text-xs text-violet-700">
        ...
        <span><b>↑ ↓</b> navigate rows &nbsp;·&nbsp; <b>Enter</b> move down &nbsp;·&nbsp; <b>Tab</b> next field &nbsp;·&nbsp; <b>Shift+Enter</b> new line</span>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        {(() => {
          ...
          const renderRow = (r: ResultRow, i: number) => {
            const computed = (r.flag?.startsWith('critical') ? r.flag : autoFlagForRow(r))
            ...
            return (
              <tr key={r.id} className={`border-b border-slate-100 transition-colors ${isFocused ? 'bg-violet-50/50' : ''} ${r.isCalculated ? 'bg-violet-50/30' : ''} ${isCritical ? 'bg-rose-50/50' : ''} ${isAbnormal ? 'bg-amber-50/30' : ''}`}>
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1.5">
                    <input value={r.test} onChange={e => setRows(rows => rows.map((x, idx) => idx === i ? { ...x, test: e.target.value } : x))} className={`w-full rounded-lg border px-2 py-1.5 text-sm ${r.lockedMeta ? 'border-transparent bg-slate-50 text-slate-700 font-medium' : 'border-slate-300 bg-white'} outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-200`} readOnly={!!r.lockedMeta} />
                    {r.isCalculated && <span className="shrink-0 rounded bg-violet-100 px-1 py-0.5 text-[10px] font-bold text-violet-600" title={r.formula}>fx</span>}
                    {r.kind === 'qualitative' && <span className="shrink-0 rounded bg-sky-100 px-1 py-0.5 text-[10px] font-bold text-sky-600">Q</span>}
                  </div>
                </td>
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={e => setRows(rows => rows.map((x, idx) => idx === i ? { ...x, normal: e.target.value } : x))}
                ...
                <td className="px-3 py-2">
                  <input
                    value={r.unit || ''}
                    readOnly={!!r.lockedMeta}
                    onChange={e => setRows(rows => rows.map((x, idx) => idx === i ? { ...x, unit: e.target.value } : x))}
                ...
                <td className="px-3 py-2">
                  <input value={r.prevValue || ''} onChange={e => setRows(rows => rows.map((x, idx) => idx === i ? { ...x, prevValue: e.target.value } : x))} className="w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm font-semibold outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-200" placeholder="—" />
                </td>
                <td className="px-3 py-2">
                  {r.isCalculated ? (
                    <input
                    ...
                    >
                      <option value="">—</option>
                      {(r.qualitativeOptions?.length ? r.qualitativeOptions : ['Positive', 'Negative', 'Reactive', 'Non-reactive']).map(opt => (
                    ...
                    <input
                      ...
                      className={`w-full rounded-lg border px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-200 disabled:opacity-60 ${
                        computed === 'critical_low' ? 'border-rose-400 bg-rose-50 font-bold text-rose-700 focus:border-rose-500' :
                        computed === 'critical_high' ? 'border-rose-400 bg-rose-50 font-bold text-rose-700 focus:border-rose-500' :
                        computed === 'abnormal_low' ? 'border-amber-300 bg-amber-50 font-semibold text-amber-800 focus:border-amber-500' :
                        computed === 'abnormal_high' ? 'border-amber-300 bg-amber-50 font-semibold text-amber-800 focus:border-amber-500' :
                ...
                <td className="px-3 py-2">
                  <div className={`w-full rounded-lg border px-2 py-1.5 text-[11px] font-bold text-center ${
                    isCritical && isLow ? 'border-rose-300 bg-rose-100 text-rose-700' :
                    isCritical && isHigh ? 'border-rose-300 bg-rose-100 text-rose-700' :
                    isCritical ? 'border-rose-300 bg-rose-100 text-rose-700' :
                    isAbnormal && isLow ? 'border-amber-300 bg-amber-100 text-amber-700' :
                    isAbnormal && isHigh ? 'border-amber-300 bg-amber-100 text-amber-700' :
                    isAbnormal ? 'border-amber-300 bg-amber-100 text-amber-700' :
                    computed === 'normal' ? 'border-emerald-300 bg-emerald-100 text-emerald-700' :
                    'border-slate-200 bg-slate-50 text-slate-400'
                  }`}>
                    {computed === 'critical_low' ? 'Critically Low' :
                     computed === 'critical_high' ? 'Critically High' :
                     computed === 'abnormal_low' ? 'Abnormal Low' :
                     computed === 'abnormal_high' ? 'Abnormal High' :
                     computed === 'normal' ? 'Normal' :
                     computed || '—'}
                  </div>
                </td>
                <td className="px-3 py-2">
                  <input value={r.comment || ''} onChange={e => setRows(rows => rows.map((x, idx) => idx === i ? { ...x, comment: e.target.value } : x))} disabled={isLocked} className="w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-200 disabled:opacity-60" placeholder="—" />
                </td>
                <td className="px-3 py-2 text-right">
                  <button onClick={() => removeRow(r.id)} className="rounded-lg p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-colors" title="Remove row">
                    <X className="h-4 w-4" />
                  </button>
                </td>
              </tr>
          ...
          const renderTable = (sectionRows: ResultRow[], showHeader = true) => (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[800px] text-sm">
                ...
                <tbody>
                  {sectionRows.length === 0 && (
                    <tr>
                      <td colSpan={8} className="px-3 py-6 text-center text-sm text-slate-400">No parameters in this section.</td>
                    </tr>
                  )}
                  {sectionRows.map((r) => {
          ...
          const autoInterp = rows.reduce<string[]>((acc, r) => {
            ...
            for (const rule of r.interpretationRules) {
              const expr = rule.expression.trim().toLowerCase().replace(/^value\s*/, '')
              let match = false
              const range = expr.match(/^(-?\d+(?:\.\d+)?)\s*(?:\.\.|-)\s*(-?\d+(?:\.\d+)?)$/)
              if (range) { const lo = Math.min(+range[1],+range[2]); const hi = Math.max(+range[1],+range[2]); match = num >= lo && num <= hi }
              const cmp = expr.match(/^(>=|<=|>|<|==|=|!=)\s*(-?\d+(?:\.\d+)?)$/)
              if (cmp) { const n = +cmp[2]; const op = cmp[1]; match = op==='>='?num>=n:op==='<='?num<=n:op==='>'?num>n:op==='<'?num<n:op==='='||op==='=='?num===n:op==='!='?num!==n:false }
              if (match) { acc.push(`${r.test}: ${rule.label || rule.text || rule.expression}`); break }
            }
            return acc
          }, [])
            ...
            <>
              ...
              {sectionDefs.length > 0 ? (
                <>
                  ...
                  {Object.entries(sectionGroups).filter(([k]) => !sectionDefs.some(s => s.key === k)).map(([k, secRows]) => (
                    <div key={k} className="mb-2">
                      <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50/60 px-4 py-2">
                        <ChevronDown className="h-3 w-3 text-slate-400" />
                        <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{k}</span>
                      </div>
                      {renderTable(secRows, false)}
                    </div>
                  ))}
              ...
              {autoInterp.length > 0 && (
                <div className="mx-4 mb-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm">
                  <div className="text-xs font-bold uppercase text-emerald-700">Auto-Interpretation</div>
                  <ul className="mt-1 list-disc pl-5 text-slate-700">
                    {autoInterp.map((a, i) => <li key={i}>{a}</li>)}
                  </ul>
                </div>
              )}
            </>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <div className="text-sm font-bold text-slate-800">Interpretation</div>
          <div className="flex flex-wrap items-center gap-2">
            <ChatGPTHandoffButton
              testName={selectedTestId ? (testsMap[selectedTestId]?.name || '') : ''}
              patient={{ fullName: patient.fullName, age: patient.age, gender: patient.gender, mrn: patient.mrn }}
              rows={rows.map(r => ({ test: r.test, value: r.value, unit: r.unit, normal: r.normal, flag: r.flag }))}
            ...
            ><Clock className="h-3 w-3" /> History</button>
            {existingResultId && (
              <button
                type="button"
              ...
              ><AlertTriangle className="h-3 w-3" /> Repeat</button>
            )}
          </div>
        </div>
        <textarea value={interpretation} onChange={e => setInterpretation(e.target.value)} disabled={isLocked} className="h-28 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200 resize-none disabled:opacity-60" placeholder="Clinical interpretation..." />
      </div>
      ...
      {showRepeat && existingResultId && (
        <RepeatSampleModal
          ...
          onDone={() => { setExistingResultId(null); setRows(rows.map(r => ({ ...r, value: '', flag: undefined as any }))); setInterpretation(''); }}
        />
      )}
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex items-center justify-between">
          <div className="text-sm font-bold text-slate-800">Previous Results{selectedTestId ? ` — ${testsMap[selectedTestId]?.name || ''}` : ''}</div>
        ...
        {prior.length > 0 && (
          <div className="space-y-3">
            {prior.map((p) => {
              ...
              return (
                <div key={p.result.id} className="overflow-hidden rounded-lg border border-slate-200">
                  <div className="flex items-center justify-between gap-2 border-b border-slate-100 bg-slate-50/60 px-3 py-2 text-xs">
                    ...
                    <button onClick={() => {
                      previewLabReportPdf({
                        ...
                        approvedAt: (p.result as any).approvedAt || undefined,
                        approvedBy: (p.result as any).approvedBy || undefined,
                        ...
                        const tokenId = (p.order as any).tokenId || (p.result as any).tokenId || token
                  ...
                  <div className="overflow-x-auto p-3">
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50 text-slate-500">
                        <tr>
                          <th className="px-2 py-1 text-left font-semibold">Test</th>
                          <th className="px-2 py-1 text-left font-semibold">Value</th>
                          <th className="px-2 py-1 text-left font-semibold">Unit</th>
                          <th className="px-2 py-1 text-left font-semibold">Normal</th>
                      ...
                      <tbody>
                        {(p.result.rows||[]).map((r:any, i:number) => (
                          <tr key={i} className="border-b border-slate-100">
                            <td className="px-2 py-1 font-medium text-slate-700">{r.test}</td>
                            <td className="px-2 py-1">{r.value || '-'}</td>
                            <td className="px-2 py-1 text-slate-500">{r.unit || '-'}</td>
                            <td className="px-2 py-1 text-slate-500">{r.normal || '-'}</td>
                            <td className="px-2 py-1">
                              {r.flag ? (
                                <span className={`inline-flex rounded px-1.5 py-0.5 text-[10px] font-bold ${
                                  String(r.flag).startsWith('critical') ? 'bg-rose-100 text-rose-700' :
                                  String(r.flag).startsWith('abnormal') ? 'bg-amber-100 text-amber-700' :
                                  'bg-emerald-100 text-emerald-700'
                                }`}>{
                                  r.flag === 'critical_low' ? 'Critically Low' :
                                  r.flag === 'critical_high' ? 'Critically High' :
                                  r.flag === 'abnormal_low' ? 'Abnormal Low' :
                                  r.flag === 'abnormal_high' ? 'Abnormal High' :
                                  r.flag === 'normal' ? 'Normal' :
                                  String(r.flag)
                                }</span>
                              ) : '-'}
                            </td>
                    ...
                    {p.result.interpretation && (
                      <div className="mt-2 whitespace-pre-wrap text-xs text-slate-600"><span className="font-semibold text-slate-700">Interpretation:</span> {p.result.interpretation}</div>
                    )}
                  </div>
                </div>
              )
      ...
      <div className="sticky bottom-0 z-10 -mx-4 -mb-4 border-t border-slate-200 bg-white/95 backdrop-blur px-4 py-3 md:-mx-6 md:-mb-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <button onClick={() => { if (returnTo === 'report-approval') { navigate('/lab/report-approval') } else { setSelected(null); setSelectedTestId(null) } }} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">
              <ChevronLeft className="h-4 w-4" /> Back
            </button>
            <button onClick={() => {
              previewLabReportPdf({
                ...
                const tokenId = selected.id || (selected as any)._id || tokenNo
                labApi.markReportPrinted(tokenId).catch(() => {})
              })
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => save()} disabled={isLocked || saving} className="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-violet-200 hover:bg-violet-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <Save className={`h-4 w-4 ${saving ? 'animate-pulse' : ''}`} /> {saving ? 'Saving...' : 'Save Draft'}
            </button>
            <button onClick={() => save({ strict: true })} disabled={isLocked || saving} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-emerald-200 hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <Send className={`h-4 w-4 ${saving ? 'animate-pulse' : ''}`} /> {saving ? 'Submitting...' : 'Submit'}
            </button>
          </div>
        </div>
      </div>
      ...
      {saveError && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm">
          <div className="flex items-center gap-2 font-bold text-rose-700">
            ...
            {saveError.code === 'TOTAL_PERCENT_MISMATCH' ? 'Differential total ≠ 100%' : 'Save failed'}
          </div>
          ...
          {saveError.validation?.groups && (
            <ul className="mt-2 list-disc pl-5 text-xs text-rose-600">
              {Object.entries<any>(saveError.validation.groups).map(([k, v]) => (
                <li key={k}>{k}: total {v.total}% (need 100 ± 0.5)</li>
              ))}
            </ul>
          )}
        </div>
      ...
      {pendingCriticalEvents.length > 0 && (
        <CriticalResultDetailModal
          ...
          onDone={() => {
            ...
            setTick(t => t + 1)
            if (returnTo === 'report-approval') navigate('/lab/report-approval')
            else { setSelected(null); setSelectedTestId(null); setExistingResultId(null) }
          }}
        />
      )}
