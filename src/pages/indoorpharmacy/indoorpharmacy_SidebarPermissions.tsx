...
import { indoorPharmacySidebarNav } from '../../components/indoorpharmacy/indoorpharmacy_Sidebar'
...
export default function Pharmacy_SidebarPermissions() {
  ...
  const navItems = (() => {
    const out: any[] = []
    for (const x of (indoorPharmacySidebarNav as any[])) {
  ...
  const loadPermissions = async () => {
    ...
    try {
      try {
        const r = await indoorPharmacyApi.listSidebarRoles() as any
      ...
      const res = await indoorPharmacyApi.listSidebarPermissions()
  ...
  const doConfirmDeleteRole = async () => {
    ...
    if (!role) return
    try {
      await indoorPharmacyApi.deleteSidebarRole(role)
  ...
  const savePermissions = async () => {
    ...
    setSaving(true)
    try {
      await indoorPharmacyApi.updateSidebarPermissions(selectedRole, {
  ...
  const doConfirmReset = async () => {
    ...
    setSaving(true)
    try {
      await indoorPharmacyApi.resetSidebarPermissions(role)
      await loadPermissions()
