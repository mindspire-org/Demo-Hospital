...
import IndoorPharmacy_AddCustomer, { type Customer } from '../../components/indoorpharmacy/indoorpharmacy_AddCustomer'
import IndoorPharmacy_EditCustomer from '../../components/indoorpharmacy/indoorpharmacy_EditCustomer'
import IndoorPharmacy_CustomerCard from '../../components/indoorpharmacy/indoorpharmacy_CustomerCard'
import IndoorPharmacy_PayBill from '../../components/indoorpharmacy/indoorpharmacy_PayBill'
...
export default function IndoorPharmacy_Customers() {
  ...
  const exportCsv = async () => {
    try {
      ...
      const first: any = await indoorPharmacyApi.listCustomers({ q: q || undefined, page: 1, limit: pageLimit })
      ...
      if (tp > 1) {
        ...
        const results = await Promise.all(pages.map(p => indoorPharmacyApi.listCustomers({ q: q || undefined, page: p, limit: effLimit })))
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res: any = await indoorPharmacyApi.listCustomers({ q: q || undefined, page, limit })
        if (!mounted) return
  ...
  return (
    <div className="space-y-6">
      ...
      <IndoorPharmacy_AddCustomer
        open={addOpen}
        onClose={() => setAddOpen(false)}
        onSave={async (c) => {
          await indoorPharmacyApi.createCustomer({
            name: c.name,
            company: c.company,
            phone: c.phone,
            address: c.address,
      ...
      />
      <IndoorPharmacy_EditCustomer
        open={editOpen}
        customer={editing}
        onClose={()=>{ setEditOpen(false); setEditing(null) }}
        onSave={async (c)=>{
          await indoorPharmacyApi.updateCustomer(c.id, {
            name: c.name,
            company: c.company,
            phone: c.phone,
            address: c.address,
      ...
      <IndoorPharmacy_ConfirmDialog
        ...
        onConfirm={async ()=>{
          if (!deleting) return
          await indoorPharmacyApi.deleteCustomer(deleting.id)
          try {
            const res: any = await indoorPharmacyApi.listCustomers({ q: q || undefined, page, limit })
            const mapped: Customer[] = (res.items || []).map((x: any) => ({
              id: x._id,
      ...
      />
      <IndoorPharmacy_PayBill open={payBillOpen} onClose={() => { setPayBillOpen(false); setPaying(null) }} customer={paying} />
