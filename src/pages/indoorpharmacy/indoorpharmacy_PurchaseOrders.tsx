...
import IndoorPharmacy_AddPurchaseOrderDialog from '../../components/indoorpharmacy/indoorpharmacy_AddPurchaseOrderDialog';
import IndoorPharmacy_ConfirmDialog from '../../components/indoorpharmacy/indoorpharmacy_ConfirmDialog';

export default function IndoorPharmacy_PurchaseOrders() {
  ...
  const loadOrders = async () => {
    try {
      const res: any = await indoorPharmacyApi.listPurchaseOrders({ q: query, page, limit });
      setOrders(res.items || []);
  ...
  const handleCreate = async (data: any) => {
    try {
      await indoorPharmacyApi.createPurchaseOrder(data);
      loadOrders();
    } catch (error) {
  ...
  const handleUpdate = async (data: any) => {
    try {
      ...
      await indoorPharmacyApi.updatePurchaseOrder(selectedOrder._id, data);
      loadOrders();
    } catch (error) {
  ...
  const handleMarkAsSent = async (id: string) => {
    try {
      await indoorPharmacyApi.updatePurchaseOrderStatus(id, 'Sent');
      loadOrders();
    } catch (error) {
  ...
  const handleMarkAsComplete = async (id: string) => {
    try {
      await indoorPharmacyApi.updatePurchaseOrderStatus(id, 'Complete');
      loadOrders();
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (!toDelete) return;
    try {
      await indoorPharmacyApi.deletePurchaseOrder(toDelete);
      loadOrders();
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      {/* Dialogs */}
      <IndoorPharmacy_AddPurchaseOrderDialog
        open={addOpen}
