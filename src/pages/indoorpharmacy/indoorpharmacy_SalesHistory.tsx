...
export default function IndoorIndoorPharmacy_SalesHistory() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res: any = await indoorPharmacyApi.listSales({ bill: bill || undefined, medicine: medicine || undefined, phone: phone || undefined, user: user || undefined, from: from || undefined, to: to || undefined, page, limit })
        if (!mounted) return
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await indoorPharmacyApi.listUsers()
        if (!mounted) return
