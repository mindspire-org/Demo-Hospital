...
import IndoorPharmacy_CashMovementSlipDialog, { type CashMovementEntry } from '../../components/indoorpharmacy/indoorpharmacy_CashMovementSlipDialog'
...
export default function IndoorPharmacy_PayInOut(){
  ...
  async function fetchMovements(p=page, l=limit){
    setLoading(true)
    try {
      const res = await indoorPharmacyApi.listCashMovements({ from: from || undefined, to: to || undefined, search: search || undefined, type: typeFilter==='Any'? undefined : typeFilter, page: p, limit: l })
  ...
  async function fetchSummary(){
    try {
      const s = await indoorPharmacyApi.cashMovementSummary({ from: from || undefined, to: to || undefined })
  ...
  const add = async () => {
    ...
    let created: any
    try {
      created = await indoorPharmacyApi.createCashMovement({
        date: date || todayStr(),
        type,
  ...
  return (
    <div className="w-full p-3 sm:p-4">
      ...
      <IndoorPharmacy_CashMovementSlipDialog open={openSlip} onClose={()=>setOpenSlip(false)} entry={slipEntry} />
