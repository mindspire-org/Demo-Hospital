...
import IndoorPharmacy_AddCompanyDialog, { type Company } from '../../components/indoorpharmacy/indoorpharmacy_AddCompanyDialog'
import { indoorPharmacyApi } from '../../utils/api'

export default function IndoorPharmacy_Companies(){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await indoorPharmacyApi.listCompanies({ q: query || undefined, page, limit })
        if (!mounted) return
  ...
  const addCompany = async (c: Company) => {
    try {
      await indoorPharmacyApi.createCompany({ name: c.name, status: c.status })
      setReloadTick(t=>t+1)
  ...
  const saveEdit = async (c: Company & { id: string }) => {
    try {
      await indoorPharmacyApi.updateCompany(c.id, { name: c.name, status: c.status })
      setReloadTick(t=>t+1)
    } catch (e) { console.error(e) }
  }

  const remove = async (id: string) => {
    try { await indoorPharmacyApi.deleteCompany(id); setReloadTick(t=>t+1) } catch (e) { console.error(e) }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Dialogs */}
      <IndoorPharmacy_AddCompanyDialog open={addOpen} onClose={()=>setAddOpen(false)} onSave={addCompany} />
      {selected && (
        <IndoorPharmacy_AddCompanyDialog open={editOpen} onClose={()=>{ setEditOpen(false); setSelected(null) }} onSave={(c)=>saveEdit({ ...c, id: selected.id })} initial={{ id: selected.id, name: selected.name, status: (selected.status||'Active') as any }} title="Edit Company" submitLabel="Save" />
      )}
    </div>
  )
}
