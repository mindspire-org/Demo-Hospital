...
export default function IndoorPharmacy_Reports() {
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try{
        const res: any = await indoorPharmacyApi.listShifts()
        if (!mounted) return
    ...
    async function load(){
      try {
        ...
        const [sales, cash, credit, inv, pur, exp, settings] = await Promise.all([
          indoorPharmacyApi.salesSummary({ from: effectiveWindow.from, to: effectiveWindow.to }),
          indoorPharmacyApi.salesSummary({ payment: 'Cash', from: effectiveWindow.from, to: effectiveWindow.to }),
          indoorPharmacyApi.salesSummary({ payment: 'Credit', from: effectiveWindow.from, to: effectiveWindow.to }),
          indoorPharmacyApi.inventorySummary(),
          indoorPharmacyApi.purchasesSummary({ from: dateFrom, to: dateTo }),
          indoorPharmacyApi.expensesSummary({ from: effectiveWindow.from, to: effectiveWindow.to }),
          indoorPharmacyApi.getSettings().catch(() => ({ pharmacyName: 'Pharmacy' })),
        ])
        if (!mounted) return
        ...
        const monthSales: any = await indoorPharmacyApi.salesSummary({ from: fmt(startMonth), to })
      ...
      try {
        const list: any = await indoorPharmacyApi.listSales({ from: effectiveWindow.from, to: effectiveWindow.to, limit: 500 })
