...
import IndoorPharmacy_CashCountSlipDialog, { type CashCountEntry } from '../../components/indoorpharmacy/indoorpharmacy_CashCountSlipDialog'
...
export default function IndoorPharmacy_ManagerCashCount(){
  ...
  async function fetchCounts(p=page, l=limit){
    setLoading(true)
    try {
      const res = await indoorPharmacyApi.listCashCounts({ from: from || undefined, to: to || undefined, search: search || undefined, page: p, limit: l })
  ...
  async function fetchSummary(){
    try {
      const s = await indoorPharmacyApi.cashCountSummary({ from: from || undefined, to: to || undefined, search: search || undefined })
  ...
  const add = async () => {
    ...
    let created: any
    try {
      created = await indoorPharmacyApi.createCashCount({
        date: date || todayStr(),
  ...
  return (
    <div className="w-full p-3 sm:p-4">
      ...
      <IndoorPharmacy_CashCountSlipDialog open={openSlip} onClose={()=>setOpenSlip(false)} entry={slipEntry} />
