...
import IndoorPharmacy_PurchaseSlipDialog from '../../components/indoorpharmacy/indoorpharmacy_PurchaseSlipDialog'
...
export default function IndoorPharmacy_PurchaseHistory() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res: any = await indoorPharmacyApi.listPurchases({ from: from || undefined, to: to || undefined, search: search || undefined, page, limit })
  ...
  const exportCsv = async () => {
    try {
      ...
      const first: any = await indoorPharmacyApi.listPurchases({ from: from || undefined, to: to || undefined, search: search || undefined, page: 1, limit: pageLimit })
      let items: any[] = (first.items || first || [])
      const tp = Number(first.totalPages || 1)
      if (tp > 1) {
        const pages = Array.from({ length: tp - 1 }, (_, i) => i + 2)
        const results = await Promise.all(pages.map(p => indoorPharmacyApi.listPurchases({ from: from || undefined, to: to || undefined, search: search || undefined, page: p, limit: pageLimit })))
  ...
  return (
    <div className="space-y-4">
      ...
      <IndoorPharmacy_PurchaseSlipDialog open={slipOpen} onClose={()=>setSlipOpen(false)} row={selected as any} />
      <IndoorPharmacy_ConfirmDialog
        ...
        onConfirm={async()=>{
          ...
          try {
            await indoorPharmacyApi.deletePurchase(toDelete.id)
