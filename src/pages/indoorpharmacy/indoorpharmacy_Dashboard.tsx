...
export default function IndoorPharmacy_Dashboard() {
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      const tasks = await Promise.allSettled([
        indoorPharmacyApi.inventorySummaryCached(undefined, { ttlMs: 120_000, forceRefresh: tick>0 }),
        indoorPharmacyApi.purchasesSummaryCached({ from: fromMonth, to: fromToday }, { ttlMs: 120_000, forceRefresh: tick>0 }),
        indoorPharmacyApi.dispenseSummaryCached({ from: fromToday, to: fromToday }, { ttlMs: 60_000, forceRefresh: tick>0 }),
        indoorPharmacyApi.dispenseSummaryCached({ from: fromMonth, to: fromToday }, { ttlMs: 120_000, forceRefresh: tick>0 }),
        indoorPharmacyApi.dispenseSummaryCached({ payment: 'Cash', from: fromToday, to: fromToday }, { ttlMs: 60_000, forceRefresh: tick>0 }),
        indoorPharmacyApi.dispenseSummaryCached({ payment: 'Credit', from: fromToday, to: fromToday }, { ttlMs: 60_000, forceRefresh: tick>0 }),
        indoorPharmacyApi.listDispensesCached({ limit: 5 }, { ttlMs: 60_000, forceRefresh: tick>0 }),
      ])
      ...
      if (inv){
        ...
        setExpiringSoon(arr.map((it:any)=> ({ name: it.name, expiry: String(it.earliestExpiry||'').slice(0,10), onHand: Number(it.onHand||0) })))
        try { localStorage.setItem('indoorpharmacy.inventory.summary', JSON.stringify({ stats: inv?.stats, expiringSoonItems: arrRaw, at: Date.now() })) } catch {}
      }
