...
export default function IndoorPharmacy_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setError('')
      const res: any = await indoorPharmacyApi.loginUser(username.trim(), password)
      try {
        localStorage.setItem('indoorpharmacy.user', JSON.stringify(res?.user || { username }))
        localStorage.setItem('indoorpharma_user', username.trim())
        const token = res?.token || res?.user?.token || res?.accessToken || ''
        if (token) localStorage.setItem('indoorpharmacy.token', token)
      } catch {}
      try {
        await indoorPharmacyApi.createAuditLog({
          actor: username || 'indoorpharmacy',
          action: 'Login',
          label: 'LOGIN',
          method: 'POST',
          path: '/indoor-pharmacy/users/login',
        ...
        <div className="card-3d relative w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-2xl">
          ...
          <div className="relative">
            <div className="relative p-8 pb-6 text-center">
              ...
              <p className="text-sm text-white/60 font-medium">Indoor Pharmacy Management System</p>
            </div>
