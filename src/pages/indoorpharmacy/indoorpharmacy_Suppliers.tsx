...
import IndoorPharmacy_AddSupplierDialog, { type Supplier } from '../../components/indoorpharmacy/indoorpharmacy_AddSupplierDialog'
import IndoorPharmacy_SupplierDetailsDialog from '../../components/indoorpharmacy/indoorpharmacy_SupplierDetailsDialog'
import IndoorPharmacy_AssignSupplierCompaniesDialog from '../../components/indoorpharmacy/indoorpharmacy_AssignSupplierCompaniesDialog'
import { indoorPharmacyApi } from '../../utils/api'

export default function IndoorPharmacy_Suppliers() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await indoorPharmacyApi.listSuppliers({ q: query || undefined, page, limit })
        if (!mounted) return
        ...
        if (zeros.length){
          const updates = await Promise.all(zeros.map(async (s) => {
            try {
              const r = await indoorPharmacyApi.listSupplierPurchases(s.id)
  ...
  const addSupplier = async (s: Supplier) => {
    try {
      const created = await indoorPharmacyApi.createSupplier({
        name: s.name,
        company: s.company,
        phone: s.phone,
        address: s.address,
      ...
      if (s.companyIds && s.companyIds.length) {
        try { await indoorPharmacyApi.assignSupplierCompanies(created._id, { companyIds: s.companyIds }) } catch {}
      }
  ...
  const saveEdit = async (s: Supplier) => {
    await indoorPharmacyApi.updateSupplier(s.id, {
      name: s.name,
      company: s.company,
      phone: s.phone,
      address: s.address,
  ...
  const remove = async (id: string) => {
    await indoorPharmacyApi.deleteSupplier(id)
  ...
  const startPayment = async (s: Supplier) => {
    ...
    try {
      const res = await indoorPharmacyApi.listSupplierPurchases(s.id)
  ...
  const savePayment = async (s: Supplier) => {
    ...
    await indoorPharmacyApi.recordSupplierPayment(s.id, { amount: amt, purchaseId: paymentDraft.purchaseId, method: paymentDraft.method, note: paymentDraft.note, date: paymentDraft.date })
  ...
  return (
    <div className="space-y-4">
      ...
      <IndoorPharmacy_AddSupplierDialog open={addOpen} onClose={()=>setAddOpen(false)} onSave={addSupplier} />
      <IndoorPharmacy_AddSupplierDialog open={editOpen} onClose={()=>setEditOpen(false)} onSave={saveEdit} initial={selected ?? undefined} title="Edit Supplier" submitLabel="Save" />
      <IndoorPharmacy_SupplierDetailsDialog open={detailsOpen} onClose={()=>setDetailsOpen(false)} supplier={selected} />
      <IndoorPharmacy_AssignSupplierCompaniesDialog open={assignCompaniesOpen} onClose={()=>setAssignCompaniesOpen(false)} supplier={selected} />
    </div>
  )
}
