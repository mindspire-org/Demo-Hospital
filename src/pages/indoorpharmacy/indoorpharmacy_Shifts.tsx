...
export default function IndoorPharmacy_Shifts(){
  ...
  useEffect(()=>{
    ...
    ;(async () => {
      try {
        const res = await indoorPharmacyApi.listShifts()
        if (!mounted) return
  ...
  const addShift = async () => {
    const created = await indoorPharmacyApi.createShift({ name: `Shift ${shifts.length+1}`, start: '09:00', end: '17:00', absentCharges: 0, lateDeduction: 0, earlyOutDeduction: 0 })
    setShifts(s => [...s, {
      id: created._id,
      name: created.name,
      start: created.start,
  ...
  const removeShift = async (id: string) => {
    await indoorPharmacyApi.deleteShift(id)
  ...
  const save = async () => {
    setSaving(true)
    try {
      await Promise.all(shifts.map(sh => indoorPharmacyApi.updateShift(sh.id, {
        name: sh.name,
        start: sh.start,
        end: sh.end,
  ...
  return (
    <div className="space-y-4">
      <div className="text-xl font-bold text-slate-800">Shifts</div>

      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex items-center gap-2">
          <div className="text-lg font-semibold text-slate-800">Shift Management</div>
