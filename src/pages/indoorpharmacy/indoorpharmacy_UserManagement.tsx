...
import { indoorPharmacyApi, financeApi } from '../../utils/api'
import Toast from '../../components/ui/Toast'

type User = { _id: string; username: string; role: string }

export default function IndoorPharmacy_UserManagement() {
  ...
  async function load(){
    ...
    try {
      const [rolesRes, usersRes] = await Promise.allSettled([
        indoorPharmacyApi.listSidebarRoles() as any,
        indoorPharmacyApi.listUsers() as any,
      ])
  ...
  const addUser = async () => {
    ...
    try {
      const created = await indoorPharmacyApi.createUser({ username: newUsername, role: newRole, password: newPassword }) as any
  ...
  const removeUser = async (_id: string) => {
    try {
      await indoorPharmacyApi.deleteUser(_id)
  ...
  const openEdit = (u: User) => setEditing({ _id: u._id, username: u.username, role: u.role, password: '' })
  const saveEdit = async () => {
    if (!editing) return
    setSavingEdit(true)
    try {
      const payload: any = { username: editing.username, role: editing.role }
      if (editing.password && editing.password.trim()) payload.password = editing.password
      const updated = await indoorPharmacyApi.updateUser(editing._id, payload) as any
  ...
  const createRole = async () => {
    ...
    try {
      await indoorPharmacyApi.createSidebarRole(role)
        ...
        <div className="w-full max-w-md overflow-hidden rounded-2xl border border-white/20 bg-white/90 shadow-2xl backdrop-blur dark:border-slate-700 dark:bg-slate-900/90">
          ...
          <div className="space-y-3">
            ...
            <div className="px-5">
              ...
              <input type="password" value={editing.password || ''} onChange={e=>setEditing(prev => prev ? { ...prev, password: e.target.value } : prev)} placeholder="Leave blank to keep current" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-200/60 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:ring-sky-900/40" />
            </div>
          </div>
