...
import IndoorPharmacy_POSCart from '../../components/indoorpharmacy/indoorpharmacy_POSCart'
...
export default function IndoorPharmacy_POS() {
  ...
  const refreshHeld = async ()=>{
    try { const r:any = await indoorPharmacyApi.listHoldSales(); setHeldBills(r?.items||[]) } catch { setHeldBills([]) }
  }
  ...
  const holdBill = async ()=>{
    ...
    try {
      ...
      await indoorPharmacyApi.createHoldSale(payload)
      setCart([])
  ...
  const restoreHeld = async (id: string)=>{
    try {
      const r:any = await indoorPharmacyApi.getHoldSale(id)
      const doc = r || {}
      ...
      await indoorPharmacyApi.deleteHoldSale(id)
      await refreshHeld()
      setHeldOpen(false)
    } catch { showToast('error', 'Failed to restore held bill') }
  }
  const deleteHeld = async (id: string)=>{ try { await indoorPharmacyApi.deleteHoldSale(id); await refreshHeld() } catch {} }
  ...
  const refreshCartStocks = async () => {
    try {
      ...
      for (const l of cart) {
        try {
          const res: any = await indoorPharmacyApi.listInventory({ search: l.name, page: 1, limit: 1 })
      ...
      try {
        ...
        for (const ln of lines) {
          ...
          if (!product) {
            ...
            try {
              const inv: any = await indoorPharmacyApi.listInventory({ search: ln.name, page: 1, limit: 1 })
  ...
  return (
    <div className={"grid gap-4 lg:grid-cols-3"}>
      ...
      <div className="space-y-4">
        ...
        <IndoorPharmacy_ProcessPaymentDialog open={payOpen} onClose={()=>setPayOpen(false)} onConfirm={confirmPayment} />
        <IndoorPharmacy_POSReceiptDialog
          open={receiptOpen}
