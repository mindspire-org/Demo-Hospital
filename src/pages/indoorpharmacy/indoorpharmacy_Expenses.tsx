...
import IndoorPharmacy_AddExpenseDialog from '../../components/indoorpharmacy/indoorpharmacy_AddExpenseDialog'
import { indoorPharmacyApi } from '../../utils/api'
import IndoorPharmacy_SalarySlipDialog from '../../components/indoorpharmacy/indoorpharmacy_SalarySlipDialog'
...
export default function IndoorPharmacy_Expenses() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await indoorPharmacyApi.listExpenses({ from: from || undefined, to: to || undefined, minAmount, search: search || undefined, type: etype === 'All Types' ? undefined : etype, user: user || undefined, page, limit })
        if (!mounted) return
  ...
  const remove = async (id: string) => {
    await indoorPharmacyApi.deleteExpense(id)
    refresh()
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <IndoorPharmacy_AddExpenseDialog
        open={addOpen}
        onClose={() => setAddOpen(false)}
        onSave={async (exp) => {
          await indoorPharmacyApi.createExpense({ ...exp, createdBy: currentUser })
          setAddOpen(false)
          setPage(1)
          refresh()
        }}
      />
      <IndoorPharmacy_SalarySlipDialog open={slipOpen} onClose={()=>{ setSlipOpen(false); setSlipExp(null) }} expense={slipExp || undefined as any} />
    </div>
  )
}
