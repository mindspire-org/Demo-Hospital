...
import IndoorPharmacy_InventoryTable from '../../components/indoorpharmacy/indoorpharmacy_InventoryTable'
import IndoorPharmacy_UpdateStock from '../../components/indoorpharmacy/indoorpharmacy_UpdateStock'
import { indoorPharmacyApi } from '../../utils/api'
import IndoorPharmacy_EditInventoryItem from '../../components/indoorpharmacy/indoorpharmacy_EditInventoryItem'
import IndoorPharmacy_ConfirmDialog from '../../components/indoorpharmacy/indoorpharmacy_ConfirmDialog'

export default function IndoorPharmacy_Inventory() {
  ...
  const approveOne = async (id: string) => {
    ...
    indoorPharmacyApi.approvePurchaseDraft(id).finally(()=> { setRefreshTick(t=>t+1) })
  }
  const rejectOne = async (id: string) => {
    setRows(prev => prev.filter((r: any) => r.draftId !== id))
    indoorPharmacyApi.deletePurchaseDraft(id).finally(()=> setRefreshTick(t=>t+1))
  }
  const approveAll = async () => {
    ...
    Promise.all(ids.map(id => indoorPharmacyApi.approvePurchaseDraft(id).catch(()=>{}))).finally(()=> { setRefreshTick(t=>t+1) })
  }
  const rejectAll = async () => {
    if (activeTab !== 'Pending Review') return
    const ids = Array.from(new Set((rows as any[]).map(r=>r.draftId).filter(Boolean))) as string[]
    setRows(prev => prev.filter((r: any) => !ids.includes(r.draftId)))
    Promise.all(ids.map(id => indoorPharmacyApi.deletePurchaseDraft(id).catch(()=>{}))).finally(()=> setRefreshTick(t=>t+1))
  }
    ...
    const load = async () => {
      try {
        ...
        if (activeTab === 'Pending Review') {
          const res: any = await indoorPharmacyApi.listPurchaseDraftLines({ search: search || undefined, page, limit })
          ...
          const res: any = await indoorPharmacyApi.listInventoryCached({ search: search || undefined, page, limit }, { ttlMs: 60_000, forceRefresh: refreshTick>0 })
          const items: any[] = res?.items ?? res ?? []
          const tp = Number(res?.totalPages || 1)
          if (!isNaN(tp)) setTotalPages(tp)
          const mapped = (items || []).map((it: any)=>({
          ...
          if (nextPage <= (tp || 1) && !pageCache[nextPage]){
            indoorPharmacyApi.listInventoryCached({ search: search || undefined, page: nextPage, limit }, { ttlMs: 60_000 })
              .then((r:any)=>{
          ...
          const res: any = await indoorPharmacyApi.listInventoryFilteredCached({ status: status as any, search: search || undefined, page, limit }, { ttlMs: 60_000, forceRefresh: refreshTick>0 })
  ...
  return (
    <div className="space-y-6">
      ...
      <IndoorPharmacy_UpdateStock open={updateStockOpen} onClose={() => setUpdateStockOpen(false)} />
      <IndoorPharmacy_EditInventoryItem open={editOpen} onClose={()=>{ setEditOpen(false); setRefreshTick(t=>t+1) }} medicine={editMedicine} />
      <IndoorPharmacy_ConfirmDialog
        open={confirmOpen}
        ...
        onConfirm={async()=>{
          ...
          try {
            await indoorPharmacyApi.deleteInventoryItem(key)
