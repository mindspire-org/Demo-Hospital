...
export async function summary(req: Request, res: Response){
  ...
  // Use buy price for lab stock value (no sale prices in lab module)
  const stockCostValue = allItems.reduce((s:any,it:any)=> {
    const cost = Number(it.lastBuyPerUnitAfterTax || it.lastBuyPerUnit || it.avgCostPerUnit || 0)
    return s + (Number(it.onHand||0) * cost)
  }, 0)
  ...
  res.json({
    items,
    stats: {
      stockCostValue: Number(stockCostValue.toFixed(2)),
      lowStockCount,
      outOfStockCount,
      expiringSoonCount,
    },
