...
export async function create(req: Request, res: Response){
  ...
  // Sync with LabToken for result_entered status
  try {
    const orderId = String(data.orderId || '')
    if (orderId) {
      const order: any = await LabOrder.findById(orderId).lean()
      if (order?.tokenNo) {
        const token = await LabToken.findOne({ tokenNo: order.tokenNo })
        if (token && (token.status === 'sample_received' || token.status === 'converted_to_sample')) {
          token.status = 'result_entered'
          token.resultEnteredAt = new Date().toISOString()
          token.resultEnteredBy = actor
          token.resultId = String(doc._id)
          await token.save()
        }
      }
    }
  } catch (e) { console.warn('Failed to sync LabToken for result entry', e) }
...
export async function update(req: Request, res: Response){
  ...
  // Sync with LabToken for approved status
  try {
    if ((patch as any).reportStatus === 'approved') {
      ...
      if (orderId) {
        ...
        if (order?.tokenNo) {
          const token = await LabToken.findOne({ tokenNo: order.tokenNo })
          if (token && token.status === 'result_entered') {
  ...
  } catch (e) { console.warn('Failed to sync LabToken for approval', e) }
