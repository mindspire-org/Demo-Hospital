...
import { LabCompany } from '../models/Company'
...
export async function create(req: Request, res: Response) {
  ...
  try {
    await createSupplierAccount(String(s._id), s.name || 'Lab Supplier', 'lab')
  } catch {}
  try {
...
export async function listCompanies(req: Request, res: Response) {
  const distributorId = String(req.query.distributorId || '')
  const filter: any = {}
  if (distributorId) filter.distributorId = distributorId
  const items = await LabCompany.find(filter).sort({ name: 1 }).lean()
  res.json({ items })
}

export async function createCompany(req: Request, res: Response) {
  const data = req.body
  const doc = await LabCompany.create({
    name: data.name,
...
export async function updateCompany(req: Request, res: Response) {
  const { id } = req.params
  const data = req.body
  const doc = await LabCompany.findByIdAndUpdate(id, {
    name: data.name,
    distributorId: data.distributorId,
    distributorName: data.distributorName,
    status: data.status,
...
export async function removeCompany(req: Request, res: Response) {
  const { id } = req.params
  await LabCompany.findByIdAndDelete(id)
  res.json({ ok: true })
}

export async function assignCompanies(req: Request, res: Response) {
  const { id } = req.params
  const { companyIds = [], unassignIds = [] } = req.body

  // Assign companies to this supplier
  for (const companyId of companyIds) {
    await LabCompany.findByIdAndUpdate(companyId, {
      distributorId: id,
      distributorName: (await LabSupplier.findById(id))?.name || '',
    })
  }

  // Unassign companies from this supplier
  for (const companyId of unassignIds) {
    await LabCompany.findByIdAndUpdate(companyId, {
      $unset: { distributorId: 1, distributorName: 1 },
    })
  }
