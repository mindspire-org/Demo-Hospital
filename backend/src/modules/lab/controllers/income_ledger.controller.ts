...
export async function list(req: Request, res: Response) {
  ...
  const pipeline: any[] = [
    // Base: aggregate LabOrder by token
    { $match: match },
    { $sort: { createdAt: -1 } },
    ...
    },
    {
      $project: {
        ...
        performedBy: 1,
        status: { $cond: [{ $gt: ['$receivableAmount', 0] }, 'receivable', 'paid'] },
        net: 1,
        receivedAmount: 1,
        receivableAmount: 1,
    ...
    // Union with LabToken entries that are not yet converted to sample
    {
      $unionWith: {
        coll: 'lab_tokens',
        pipeline: [
          ...
          // Apply search filter on tokens as well
          ...(qd.q && String(qd.q).trim()
            ? [{
                $match: {
                  $or: [
                    { tokenNo: { $regex: String(qd.q).trim(), $options: 'i' } },
                    { 'patient.fullName': { $regex: String(qd.q).trim(), $options: 'i' } },
                    { 'patient.mrn': { $regex: String(qd.q).trim(), $options: 'i' } },
                    { 'patient.phone': { $regex: String(qd.q).trim(), $options: 'i' } },
                  ],
                },
              }] : []),
          {
            $project: {
              ...
              payments: [], // no detailed payment records stored at token stage
              tests: { $ifNull: ['$tests', []] },
  ...
  if (method) {
    pipeline.push({ $match: { payments: { $elemMatch: { method } } } })
  }
