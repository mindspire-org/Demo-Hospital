...
import { LabHoldPurchaseInvoice } from '../models/HoldPurchaseInvoice'
...
export async function getOne(req: Request, res: Response){
  const { id } = req.params
  const doc = await LabPurchaseDraft.findById(id).lean()
...
export async function update(req: Request, res: Response){
  const { id } = req.params
  const data = labDraftCreateSchema.partial().parse(req.body)
  const { lines, totals } = data.lines ? calcTotals(data.lines, (data as any).discount || 0, data.invoiceTaxes || []) : { lines: data.lines, totals: undefined }
  ...
  if (data.invoiceTaxes) updateData.invoiceTaxes = data.invoiceTaxes
  if (lines) updateData.lines = lines
  if (totals) updateData.totals = totals
  const doc = await LabPurchaseDraft.findByIdAndUpdate(id, updateData, { new: true })
  if (!doc) return res.status(404).json({ error: 'Draft not found' })
  res.json(doc)
}

export async function getNextInvoiceNumber(req: Request, res: Response){
  const last: any = await LabPurchaseDraft.findOne().sort({ createdAt: -1 }).lean()
  let next = 1
  if (last?.invoice) {
    const m = last.invoice.match(/(\d+)/)
    if (m) next = parseInt(m[1], 10) + 1
  }
  const invoiceNo = `PI-${String(next).padStart(4, '0')}`
  res.json({ invoiceNo })
}
...
export async function listHeld(req: Request, res: Response){
  const items = await LabHoldPurchaseInvoice.find().sort({ createdAt: -1 }).lean()
  res.json({ items })
}

export async function getHeld(req: Request, res: Response){
  const { id } = req.params
  const doc = await LabHoldPurchaseInvoice.findById(id).lean()
  if (!doc) return res.status(404).json({ error: 'Held invoice not found' })
  res.json(doc)
}

export async function createHeld(req: Request, res: Response){
  const data = req.body
  const doc = await LabHoldPurchaseInvoice.create({
...
export async function removeHeld(req: Request, res: Response){
  const { id } = req.params
  await LabHoldPurchaseInvoice.findByIdAndDelete(id)
  res.json({ ok: true })
}
