...
export async function summary(req: Request, res: Response){
  ...
  const [ordersInRange, tokensInRange, expensesInRange, purchasesAll] = await Promise.all([
  ...
  const totalTests = ordersInRange.reduce((s:any,o:any)=> s + (Array.isArray(o.tests)? o.tests.length : 0), 0)
    + tokensInRange.reduce((s:any,t:any)=> s + (Array.isArray((t as any).tests)? (t as any).tests.length : 0), 0)
  ...
  const tokenRevenueAgg = await LabToken.aggregate([
    { $match: tokenMatch },
    { $group: { _id: null, total: { $sum: '$net' } } },
  ])
  const totalRevenue = Number(totalRevenueAgg?.[0]?.total || 0) + Number(tokenRevenueAgg?.[0]?.total || 0)
