...
import { LabToken } from '../../lab/models/Token'
...
export async function myActivity(req: Request, res: Response){
  ...
  const [tokens, erPayments, ipdJournals, labCarts, diagnosticCarts, labTokens] = await Promise.all([
    HospitalToken.find({
      portal,
    ...
    // Lab tokens created from Reception (keep regardless of later status; exclude only if actually deleted from DB)
    LabToken.find({
      status: { $in: ['token_generated', 'converted_to_sample', 'sample_received', 'result_entered', 'approved', 'cancelled'] },
      portal,
  ...
  // For cancelled tokens, treat received as negative (refund/void)
  const labTokenTotal = (labTokens || []).reduce((s: number, t: any) => {
    const sign = String(t.status || '') === 'cancelled' ? -1 : 1
    return s + sign * Number(t.receivedAmount || 0)
  }, 0)
  const diagnosticCartTotal = (diagnosticCarts || []).reduce((s: number, o: any) => s + Number(o.receivedAmount || 0), 0)

  const inflowTotal = tokenRevenue + erTotal + ipdTotal + labCartTotal + labTokenTotal + diagnosticCartTotal
  ...
  res.json({
    ...
    summary: {
      ...
      labTokens: { count: (labTokens || []).length, total: labTokenTotal },
