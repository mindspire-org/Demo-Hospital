...
async function nextToken(date?: Date){
  ...
  // Daily counter key: resets every midnight in configured TZ
  ...
  // If key is first created today, align with existing orders created today (avoid duplicate after server restart)
  if (c && Number(c.seq) === 1) {
    try {
      // Compute start/end of the same day in TZ
      ...
      const docs: any[] = await DiagnosticOrder.find({ createdAt: { $gte: start, $lte: end }, tokenNo: { $regex: /^\d+$/ } }).select('tokenNo').lean()
...
export async function receivePayment(req: Request, res: Response){
  ...
  // Resolve the day scope for this token (daily-reset numbering)
  const tz = String((env as any).DIAGNOSTIC_TOKEN_TZ || 'Asia/Karachi')
  ...
  const endToday = new Date(startToday); endToday.setDate(startToday.getDate() + 1); endToday.setMilliseconds(endToday.getMilliseconds() - 1)
  let orders = await DiagnosticOrder.find({ tokenNo, createdAt: { $gte: startToday, $lte: endToday } }).lean()
  // If none today, pick the most recent day for this token and scope to that day
  if (!orders.length) {
    const last = await DiagnosticOrder.findOne({ tokenNo }).sort({ createdAt: -1 }).lean()
    if (!last) return res.status(404).json({ error: 'Token not found' })
    const lastStr = new Intl.DateTimeFormat('en-CA', { timeZone: tz, year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date((last as any).createdAt))
    const start = new Date(`${lastStr}T00:00:00`)
    const end = new Date(start); end.setDate(start.getDate() + 1); end.setMilliseconds(end.getMilliseconds() - 1)
    orders = await DiagnosticOrder.find({ tokenNo, createdAt: { $gte: start, $lte: end } }).lean()
  }
  ...
  const updated = await DiagnosticOrder.find({ _id: { $in: orders.map((o:any)=> o._id) } }).sort({ createdAt: 1 }).lean()
