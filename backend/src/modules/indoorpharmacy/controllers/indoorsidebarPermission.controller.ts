...
import { SidebarPermission } from '../models/indoorSidebarPermission'
import { sidebarPermissionCreateSchema, sidebarPermissionUpdateSchema, sidebarPermissionQuerySchema } from '../validators/indoorsidebarPermission'

// Default sidebar configuration
const defaultSidebarItems = [
  { path: '/indoor-pharmacy', label: 'Dashboard', order: 1 },
  { path: '/indoor-pharmacy/pos', label: 'Dispense Medicines', order: 2 },
  { path: '/indoor-pharmacy/inventory', label: 'Inventory', order: 3 },
  { path: '/indoor-pharmacy/customers', label: 'Patients', order: 4 },
  { path: '/indoor-pharmacy/suppliers', label: 'Suppliers', order: 5 },
  { path: '/indoor-pharmacy/sales-history', label: 'Dispense History', order: 6 },
  { path: '/indoor-pharmacy/purchase-history', label: 'Purchase History', order: 7 },
  { path: '/indoor-pharmacy/return-history', label: 'Return History', order: 8 },
  { path: '/indoor-pharmacy/reports', label: 'Reports', order: 13 },
  { path: '/indoor-pharmacy/guidelines', label: 'Guidelines', order: 14 },
  { path: '/indoor-pharmacy/returns', label: 'Patient Return', order: 15 },
  { path: '/indoor-pharmacy/supplier-returns', label: 'Supplier Return', order: 16 },
  { path: '/indoor-pharmacy/audit-logs', label: 'Audit Logs', order: 17 },
  { path: '/indoor-pharmacy/expenses', label: 'Expenses', order: 18 },
  { path: '/indoor-pharmacy/settings', label: 'Settings', order: 19 },
  { path: '/indoor-pharmacy/user-management', label: 'User Management', order: 20 },
]
...
const defaultVisibility = {
  ...
  pharmacist: defaultSidebarItems.map(item => ({
    ...item,
    visible: ['/indoor-pharmacy/user-management'].includes(item.path) ? false : true
  })),
  salesman: defaultSidebarItems.map(item => ({
    ...item,
    visible: ['/indoor-pharmacy/inventory', '/indoor-pharmacy/suppliers', '/indoor-pharmacy/purchase-history', '/indoor-pharmacy/reports', '/indoor-pharmacy/guidelines', '/indoor-pharmacy/supplier-returns', '/indoor-pharmacy/audit-logs', '/indoor-pharmacy/expenses', '/indoor-pharmacy/settings', '/indoor-pharmacy/user-management'].includes(item.path) ? false : true
  }))
}
...
export async function updatePermissions(req: Request, res: Response) {
  ...
  // Log the action
  try {
    const { AuditLog } = await import('../models/indoorAuditLog')
