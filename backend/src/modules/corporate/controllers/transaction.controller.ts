...
export async function list(req: Request, res: Response){
  const { companyId, serviceType, refType, refId, status, patientMrn, from, to, sort, order, page, limit } = req.query as any
  const q: any = {}
  ...
  // Build sort object from query params, default to createdAt desc
  const sortField = sort || 'createdAt'
  const sortDir = order === 'asc' ? 1 : -1
  const sortObj: any = { [sortField]: sortDir }

  const [rows, total] = await Promise.all([
    CorporateTransaction.find(q).sort(sortObj).skip(skip).limit(lim).lean(),
