...
export async function getById(req: Request, res: Response){
  ...
  // Fetch patient details for each transaction
  const transactionsWithPatients = await Promise.all(
    tx.map(async (transaction: any) => {
      let patientDetails: any = null
      
      // Try to get patient details by MRN first
      if (transaction.patientMrn) {
        try {
          const patient = await LabPatient.findOne({ mrn: transaction.patientMrn })
            .select('fullName mrn age gender phoneNormalized address')
            .lean()
  ...
  res.json({ claim, transactions: transactionsWithPatients })
}

export async function getByIdWithPatientDetails(req: Request, res: Response){
  const { id } = req.params as any
  const claim = await CorporateClaim.findById(id).lean()
  if (!claim) return res.status(404).json({ error: 'Claim not found' })
  const tx = await CorporateTransaction.find({ claimId: String(id) }).sort({ createdAt: 1 }).lean()
  
  // Group transactions by patient and fetch patient details once per unique patient
  const patientMap = new Map<string, any>()
  const uniqueMrns = [...new Set(tx.map(t => t.patientMrn).filter(Boolean))]
  
  // Fetch all unique patients at once
  if (uniqueMrns.length > 0) {
    try {
      const patients = await LabPatient.find({ mrn: { $in: uniqueMrns } })
        .select('fullName mrn age gender phoneNormalized address')
        .lean()
      
      patients.forEach(patient => {
        patientMap.set(patient.mrn, patient)
      })
    } catch (error) {
      console.error('Error fetching patient details:', error)
    }
  ...
  // Attach patient details to each transaction
  const transactionsWithPatients = tx.map(transaction => ({
    ...transaction,
    patientDetails: transaction.patientMrn ? patientMap.get(transaction.patientMrn) : null
  }))
