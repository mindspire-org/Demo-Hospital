...
export async function createDoctorPayout(
  ...
  const journal = await FinanceJournal.create({ dateIso, module: 'hospital', refType: 'doctor_payout', refId: doctorId, memo: memo || 'Doctor payout', lines })
...
export async function manualDoctorEarning(data: { doctorId: string; departmentId?: string; departmentName?: string; phone?: string; amount: number; revenueAccount?: 'OPD_REVENUE'|'PROCEDURE_REVENUE'|'IPD_REVENUE'; paidMethod?: 'Cash'|'Bank'|'AR'; memo?: string; sharePercent?: number; patientName?: string; mrn?: string }){
  const dateIso = todayIso()
  // If sharePercent not explicitly provided and revenueAccount is IPD, auto-fetch ipdShare from doctor
  let sharePercent = data.sharePercent
  if (sharePercent == null && data.doctorId) {
    try {
      const doc = await HospitalDoctor.findById(data.doctorId).lean()
      if (data.revenueAccount === 'IPD_REVENUE') {
        const ipdShr = Number((doc as any)?.ipdShare)
        const fallbackShr = Number((doc as any)?.shares ?? 100)
        sharePercent = (!isNaN(ipdShr) && ipdShr > 0) ? ipdShr : fallbackShr
      } else if (data.revenueAccount === 'OPD_REVENUE') {
        const opdShr = Number((doc as any)?.opdShare)
        const fallbackShr = Number((doc as any)?.shares ?? 100)
        sharePercent = (!isNaN(opdShr) && opdShr > 0) ? opdShr : fallbackShr
      } else {
        sharePercent = Number((doc as any)?.shares ?? 100)
      }
    } catch {
  ...
  // Calculate doctor earning = amount * (sharePercent / 100)
  const rawAmount = Math.max(data.amount || 0, 0)
  const doctorAmount = round2(rawAmount * (Math.max(0, Math.min(100, Number(sharePercent))) / 100))
  const tagsBase: any = { }
...
export async function postOpdTokenJournal(args: { tokenId: string; dateIso: string; fee: number; doctorId?: string; departmentId?: string; patientId?: string; patientName?: string; mrn?: string; tokenNo?: string; paidMethod?: 'Cash'|'Bank'|'AR'; sessionId?: string; createdByUsername?: string }){
  ...
  // Fetch doctor's share percentage: prefer opdShare, fallback to shares, default 100%
  let doctorShares = 100
  if (args.doctorId) {
    try {
      const doc = await HospitalDoctor.findById(args.doctorId).lean()
      const opdShr = Number((doc as any)?.opdShare)
      const fallbackShr = Number((doc as any)?.shares ?? 100)
      doctorShares = (!isNaN(opdShr) && opdShr > 0) ? opdShr : fallbackShr
    } catch {}
  }
  ...
  // Doctor amount = fee × (shares / 100)
  if (args.doctorId && Number(args.fee || 0) > 0 && doctorShares > 0){
    const doctorAmount = round2(Math.max(Number(args.fee || 0) * (doctorShares / 100), 0))
