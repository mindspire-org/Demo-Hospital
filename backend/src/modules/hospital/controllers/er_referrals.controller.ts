...
import { HospitalErReferral } from '../models/ErReferral'
...
export async function create(req: Request, res: Response) {
  const data = createErReferralSchema.parse(req.body)
  ...
  const serial = await nextErReferralSerial()

  const row = await HospitalErReferral.create({
    serial,
    status: 'New',
...
export async function list(req: Request, res: Response) {
  ...
  if (q.priority) crit.priority = String(q.priority)
  ...
  let rows = await HospitalErReferral.find(crit).sort({ createdAt: -1 }).lean()
...
export async function getById(req: Request, res: Response) {
  ...
  const row = await HospitalErReferral.findById(String(id)).lean()
...
export async function update(req: Request, res: Response) {
  const { id } = req.params as any
  const data = updateErReferralSchema.parse(req.body)
  const patch: any = { ...data }
  if (data.referralDate) patch.referralDate = new Date(data.referralDate)
  const row = await HospitalErReferral.findByIdAndUpdate(String(id), { $set: patch }, { new: true }).lean()
  if (!row) return res.status(404).json({ error: 'Referral not found' })
  res.json({ referral: row })
}
...
export async function updateStatus(req: Request, res: Response) {
  const { id } = req.params as any
  const data = updateErReferralStatusSchema.parse(req.body)
  const map: any = { accept: 'Accepted', reject: 'Rejected', reopen: 'New' }
  const status = map[data.action]
  const row = await HospitalErReferral.findByIdAndUpdate(String(id), { $set: { status }, $push: { statusHistory: { at: new Date(), action: data.action, note: data.note } } }, { new: true }).lean()
  if (!row) return res.status(404).json({ error: 'Referral not found' })
  res.json({ referral: row })
}

export async function startVisit(req: Request, res: Response) {
  const { id } = req.params as any
  const data = startErVisitSchema.parse(req.body)
  const ref = await HospitalErReferral.findById(String(id))
  if (!ref) return res.status(404).json({ error: 'Referral not found' })
  if ((ref as any).status === 'In-Progress') return res.status(400).json({ error: 'Already started' })
  ...
  try {
    ...
    console.warn('Failed to create token for ER referral', e)
  }
...
export async function completeVisit(req: Request, res: Response) {
  ...
  const ref = await HospitalErReferral.findById(String(id))
  if (!ref) return res.status(404).json({ error: 'Referral not found' })
  if ((ref as any).status === 'Completed') return res.status(400).json({ error: 'Already completed' })

  const encounterId = (ref as any).erEncounterId
  if (encounterId) {
    await HospitalEncounter.findByIdAndUpdate(encounterId, { $set: { status: 'completed', endAt: new Date() } })
  }

  ;(ref as any).status = 'Completed'
  ;(ref as any).statusHistory = ([...(ref as any).statusHistory || [], { at: new Date(), action: 'complete', note: '' }])
  await ref.save()
